...
After the protocol findings are walked through, if scope was `full`, run the **smell / hygiene sweep** — the first concrete review **type** of `### Review typology` in `workflow/review-typology.md`. **Read `workflow/review-typology.md` before running the sweep** (the registry; do not re-encode the type list, cadence, or engine-selection rule here — reference it by name). Before spawning the sweep subagent, apply **deployment-context enrichment** per `### Deployment-context triage` in `workflow/review-typology.md` — read `docs/architecture.md` `## Operational limits & budgets` + `## Architectural constraints` (and `docs/threat-model.md` when present) and include them in the subagent prompt so the reviewer calibrates severity for the actual deployment target. It is a **whole-codebase** review type, so its engine is **selected per the engine-selection rule in `### Review typology`**: prefer `wb-development:code-review-orchestrator` when it is available and `WB_REVIEW_ORCHESTRATOR` is not `off`, otherwise fall back to the built-in `/code-review` skill at the selected depth (below). The chosen engine runs over the **proportional scope** to surface functionality-preserving hygiene issues (dead code, duplication, high cognitive complexity, over-complexity, simplification) — distinct from a per-diff bug.
...
- **No orchestrator** → **offer** the PM (via the structured-question tool): **(a) `/code-review ultra`** — cloud, broad, **model not ours** (announce "ultra picks its own models"; the `review-full-model` pin does **not** apply), or **(b) the built-in `/code-review` over the whole codebase** — **pinnable** to `review-full-model` (cross-model works here without the orchestrator). The PM picks breadth (`ultra`) vs cross-model-pinned whole-base (built-in).

When the resolved model is `session` / equals the session / unavailable, run on the session and announce the fallback. **`ultra` is the one non-pinnable path** — on it the model knob does not apply. The engine-selection + depth behavior below is **unchanged** — this adds only the model. Read the rule by name; do not re-encode the enum/default/Haiku-blacklist/fallback/fan-out-caveat. Its deterministic **detection** half (an enumerable smell catalog a hook/linter should own) is a **named downstream/future path, not run here** (this protocol repo has no linter to host it — see `### Review typology`); the AI **prioritization / root-cause** half is this sweep.
