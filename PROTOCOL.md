...
1. **Understand.** A session resuming prior work FIRST reads the resume pointer **`.ai-dev/state/current.md`** — by that exact path, never via file-search (it hides dot-directories on some harnesses). Then the Orchestrator checks git is clean and on a fresh branch off `main`, and reads the project context it needs — **the product brief (`docs/product.md`: who the product is for and why)**, `docs/architecture.md`, the threat model (`docs/threat-model.md`) where one exists, the user journeys, the touched feature docs. Grounds the plan in the real product and the real system, not a guess; a configured project with no brief yet is offered product discovery first (setup's sibling — the orchestrator's `## Product discovery`); an existing project whose `docs/architecture.md` is absent or still the unfilled install template is offered doc bootstrap (discovery's sibling — the orchestrator's `## Doc bootstrap`); a greenfield with no tree yet is offered project inception (bootstrap's greenfield mirror — the orchestrator's `## Project inception`).
...
- **`fixup`** — the loop with plan and review collapsed into one lightweight pass, for a genuinely trivial change (a typo, a one-line fix). The Reviewer pass is **shortened, never skipped**. Single home: orchestrator's `## Fixup`.
...
`ai-dev.config.json` (project root) binds a project's choices, so the core depends on **no specific agent**:

- **mode** — `autonomous | interactive`; the value-home for invariant 7 (absent or unrecognised ⇒ `interactive`).
- **profile** — `full | lite | solo` (absent/unrecognised ⇒ `solo`): the speed↔quality dial. States only the wish for the cuttable levers (who builds, plan ceremony). The floor — independent review by a separate fresh Reviewer, the honesty gates, the merge stamp, the Operator's explicit merge authorization — holds in every profile, enforced regardless. A profile that cuts the floor is no protocol; ceremony above the change's risk is a defect the Operator pays for — the dial cuts ceremony, the floor holds rigor. Value-home: `ai-dev.config.json` `_profile`.
