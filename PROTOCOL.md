...
- The Orchestrator is the running session. The Researcher-Planner, Builder, and Reviewer are **separate spawned sub-agents** — separate contexts. This separation makes "the reviewer is independent" true and lets the enforcement layer tell an orchestrator-authored write from a sub-agent-authored one (`## Enforcement`).
...
- **Researcher-Planner.** Researches and plans, never codes. *Guarantees:* a plan the Operator can approve before any code; a structural choice surfaced for the Operator, not silently taken; the plan grounded in the real product, system, and stack; the plan precise enough for a pure executor to build without re-designing. Spawned only where the `profile` staffs the seat (`## Project config`); elsewhere the Orchestrator holds these guarantees when it plans directly.
...
- **Orchestrator.** Drives the loop and owns the contract boundary: routes each beat to its role, relays to the Operator, holds the gates — and never does a role's work itself, except where a rigor `profile` lets it plan or build directly (on guarantee profiles the Reviewer is always a separate spawn; `## Project config`).
