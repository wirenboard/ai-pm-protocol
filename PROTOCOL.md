...
1. **Understand.** A session resuming prior work FIRST reads the resume pointer **`.ai-dev/state/current.md`** — by that exact path, never via file-search (it hides dot-directories on some harnesses); absent (fresh clone or first session), fall back: `git log` for recent context + `gh pr list` for any open PR awaiting merge. Then the Orchestrator checks git is clean and on a fresh branch off `main` (no repository at all ⇒ offer the repo bootstrap first — setup's repo check, the orchestrator's `## Setup` step 0), and reads the project context it needs — **the product brief (`docs/product.md`: who the product is for and why)**, `docs/architecture.md`, the threat model (`docs/threat-model.md`) where one exists, the user journeys, the touched feature docs. Grounds the plan in the real product and the real system, not a guess; a configured project with no brief yet is offered product discovery first (setup's sibling — the orchestrator's `## Product discovery`); an existing project whose `docs/architecture.md` is absent or still the unfilled install template is offered doc bootstrap (discovery's sibling — the orchestrator's `## Doc bootstrap`); a greenfield with no tree yet is offered project inception (bootstrap's greenfield mirror — the orchestrator's `## Project inception`); a present `.ai-dev/UPGRADING.md` marker (the installer detected a version change) is offered the upgrade check (the orchestrator's `## Upgrade`); a session whose own harness differs from the config's `platform` is offered the platform switch (the orchestrator's `## Setup`, *Platform switch*).
...
4. **Review** *(guarantee profiles only).* A **freshly spawned Reviewer** — never the Builder, never a stale artifact — checks the work against the plan and its review checklist, and stamps a verdict. A failed, missing, or skipped review counts as *not reviewed* (`## Invariants`, gate integrity). On `yolo` this beat is skipped; the Operator's merge word stands in.
5. **Ship.** On the Operator's explicit go, the Orchestrator bumps the version, writes the CHANGELOG entry, pushes, and opens the PR, then **deletes every transient working artifact of this feature** — the plan, the review stamp, and any audit run for it. Delete the stamp **last**, *after* the push and PR succeed (the merge-gate reads it at push time). The durable record is the commits, the CHANGELOG, and (for a user-facing change) the contracts home (`docs/contracts/`). **Merge happens only on the Operator's explicit authorization** (per merge, never inferred) — the Operator merges, or authorizes the Orchestrator to execute it. In every autonomy mode the *decision* is the Operator's; only the *execution* is delegable.
...
- **`upgrade`** `[persona]` — executes the protocol's per-version migration notes after a tooling bump; triggered by the installer's `.ai-dev/UPGRADING.md` marker or on explicit request. Single home: orchestrator's `## Upgrade`.
...
2. **Stay inside the project** `[mechanical]`. Every role reads, searches, and writes only within the project root — no parent directories, no sibling repos. One carve-out *inside* the root: **`.ai-dev/tooling/`** (the enforcer's own source) is off-limits to read and write; the rest of `.ai-dev/` is fair game. *Prevents: a boundary breach outside the work, or an agent reading/self-patching the enforcer.*
