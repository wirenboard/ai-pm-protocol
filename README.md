...
### Скилл `code-review-orchestrator` отключён по умолчанию

Шаблон по умолчанию **запрещает** wb-* скилл `wb-development:code-review-orchestrator` (в этом проекте и в каждом downstream-проекте — через хук в `.claude/settings.json`): его широкий авто-триггер перехватывал бы собственный `/code-review` протокола (Pass-2). Запускай ревью командой `/code-review`.

Если всё же нужно запустить именно этот скилл в рамках сессии — задай переменную окружения **`WB_ALLOW_REVIEW_ORCHESTRATOR=1`**. Это **точечный, на один скилл** обход: он снимает только этот запрет, а все остальные хуки и все остальные wb-* deny остаются активны (в отличие от отключения хуков целиком через `/hooks`).

Важно: флаг должен быть выставлен **до запуска Claude Code** — хук читает окружение самого процесса Claude Code. Запускай так: `WB_ALLOW_REVIEW_ORCHESTRATOR=1 claude` или сделай `export WB_ALLOW_REVIEW_ORCHESTRATOR=1` в шелле перед стартом. `export` уже внутри запущенной сессии до хука не дойдёт и не сработает.
