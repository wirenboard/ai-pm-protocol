...
**Новая версия шаблона требует новой документации — видно.** При обновлении шаблона `/pm-audit` показывает PM, какие документационные дисциплины ввела новая версия и которые проект ещё не заполнил, и помогает заполнить их вместе с pm-architect.
...
**Протокол работает не только для кода.** Проект объявляет вид (`## Project kind: software | documentation` в `CLAUDE.md`, по умолчанию `software`): для `documentation`-проекта (SOP, runbook, гайды, спеки, диаграммы — без кода) deliverable-документы живут в отдельной директории `deliverable/`, тесты и code-review неактивны, а валидация подбирается под тип документа — сухой прогон/tabletop для actionable-доков или editorial-review + экспертный sign-off для reference-доков, как load-bearing gate; план, контракты, журналы, аудит и состояние работают как обычно.

**Решения по продукту — два режима.** По умолчанию **interactive**: PM отвечает на каждую продуктовую развилку. Опционально **autonomous**: пайплайн сам решает развилку из bootstrap-мандата и канона проекта, объявляя каждое решение и записывая его с цитатой-обоснованием для последующего batch-ревью, эскалируя только то, что канон не покрывает. Merge всегда остаётся за PM. Режим — в `.ai-pm/decision-authority.md`, по умолчанию interactive (см. `### Decision authority` в `WORKFLOW.md`).
