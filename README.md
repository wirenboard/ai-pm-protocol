...
| **D** | **Process** | На чём строим / как храним данные / как ведём разработку | `tech-stack.md`, `database-design-base.md` + per-kind, `dev-environment.md`, optional `maintenance-playbook.md`, `development-protocol-overlay.md`, ai-linting-rules, subagent configs | WRITE | SKIP |
...
- **`discipline-advisor`** — read-only 5-axis quality challenger. Hybrid floor (deterministic hard rules: PII/auth/crypto/payments detection) + smart layer (bounded scan, <10k tokens, cached per session). Opt-in пока PoC accuracy gate ≥80% per axis не достигнут — после validation mandatory в Stage F triggers
