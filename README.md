...
**Протокол работает не только для кода.** Проект объявляет вид (`## Project kind: software | process` в `CLAUDE.md`, по умолчанию `software`): для `process`-проекта (SOP, runbook, документация без кода) тесты и code-review неактивны, а валидация — сухой прогон/tabletop как load-bearing gate; план, контракты, журналы, аудит и состояние работают как обычно.
