...
**PM видит, что делает система — на одном экране.** `docs/product.md` — контракт-центричная карта: каждая фича-гарантия (контракт), под ней изменения и ревью, что её строили. Генерируется автоматически из контрактов / планов / ревью.
...
docs/
  product.md                       — что система делает: контракт → фичи (для PM, авто-генерируется)
  architecture.md
  stack-notes.md
  user-journeys.md
  features/
