...
v2.2 заменяет feature index (`docs/features/_index.md`) на контракт-центричную карту (что система делает, организовано по контрактам — что каждая фича гарантирует). Миграция — в одну команду.
...
**Шаг 2 — мигрировать карту:** скажи Claude **«мигрируй на v2.2»**. Он сгенерирует карту из существующих контрактов, планов и ревью и удалит `docs/features/_index.md`. Больше ничего не меняется.

Backend-only проект (без user-facing контрактов) — карта соберётся с одной секцией `Infrastructure`, это нормально.

## Миграция на v2.3.0

v2.3 разделяет `docs/product.md` на два файла: **авторский** `docs/product.md` (продуктовая визитка для PM, ведёт `pm-architect`) и **генерируемую** карту `docs/product-map.md` (контракт → фичи). Раньше `docs/product.md` был генерируемой картой, но назывался «Product» — теперь это две роли в двух файлах.

**Шаг 1 — обновить submodule** (как выше, `to v2.3`).

**Шаг 2 — мигрировать:** скажи Claude **«мигрируй на v2.3»**. Если `docs/product.md` — старая генерируемая карта (несёт frozen-сигнатуру генератора) и `docs/product-map.md` ещё нет: он переименует карту в `docs/product-map.md` (`git mv`, без потери содержимого) и создаст пустую авторскую `docs/product.md` из шаблона, чтобы PM заполнил визитку. Идемпотентно: повторный запуск — no-op; авторскую `product.md` без сигнатуры не трогает.
