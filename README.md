...
Главная защита от расхождения кода со спекой — **независимый reviewer**, не код-ревью человеком. Чтобы убрать цикл «открыть PR → ждать комментариев человека → fix → ждать снова», reviewer запускается **локально в той же сессии**, до `git push`. Защита построена по принципу defense-in-depth — два независимых слоя enforcement, ни один не зависит от gh CLI:

Workflow:

1. `coder` дописал implementation — есть diff изменений vs `main`. Handoff оператору: «готово к ревью + acceptance»
2. Оператор (или координирующий AI) запускает `reviewer`. Reviewer читает diff и пишет review-файл `doc/features/<topic>_review.md` со строкой `**Verdict:** approve | approve-with-comments | request-changes` в первых 50 строках (для chore / docs / fix PR'ов — `.ai-pm/.reviews/<branch>.json`)
3. Review-файл коммитится в ту же feature-ветку
4. **Layer 4 — git pre-push hook** (`.git/hooks/pre-push` через `scripts/install-git-hooks.sh`) при `git push` вызывает `scripts/check-review-trail.sh <branch>`. Парсит `Verdict:` и блокирует push если файла нет или verdict = `request-changes`. Работает в любом окружении (terminal / IDE / Claude), не зависит от gh CLI.
5. **Layer 5 — CI workflow** (`.github/workflows/check-review-trail.yml`) запускается на `pull_request` events и вызывает тот же script против PR head. Branch protection rule делает этот check **required** → merge заблокирован пока CI не green. Защищает от случаев когда pre-push hook не установлен / обойдён.
6. Обойти можно только через явный override-маркер в commit body (см. § Skip ненужного шага → Другие механики)
