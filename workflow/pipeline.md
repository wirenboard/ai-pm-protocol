...
- **Semgrep pre-check (before AI code-review):** detect the project's primary language from `docs/architecture.md ## Stack`. For each language with a known community ruleset (`p/python`, `p/javascript`, `p/typescript`, `p/go`, `p/java`), run `semgrep --config p/<lang> --json <changed-files>` on the files touched by `git diff @{upstream}...HEAD --name-only`. Append Semgrep findings (formatted as findings entries) to `## Code review findings` in `.ai-pm/reviews/<topic>_review.md`. If Semgrep is not installed, the config download fails, or no community config is known for the language, skip silently. AI `code-review` runs regardless — the pre-check is additive; all findings flow to `pm-coder` as one artifact.
