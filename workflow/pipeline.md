...
- Apply **deployment-context enrichment** per `### Deployment-context triage` in `workflow/review-typology.md` — before spawning the review subagents (code-review and seam-completeness), read `docs/architecture.md` `## Operational limits & budgets` + `## Architectural constraints` (and `docs/threat-model.md` when present) and include them in each subagent prompt so reviewers calibrate severity for the actual deployment target. Read the rule by name; do not re-encode it here.
- **Semgrep pre-check (before AI code-review):** detect the project's primary language from `docs/architecture.md ## Tech stack`. For each language with a known community ruleset (`p/python`, `p/javascript`, `p/typescript`, `p/go`, `p/java`), run `semgrep --config p/<lang> --json <changed-files>` on the files touched by `git diff @{upstream}...HEAD --name-only` (or `git diff main...HEAD` if no upstream is configured). Append Semgrep findings (formatted as findings entries) to `## Code review findings` in `.ai-pm/reviews/<topic>_review.md`. If Semgrep is not installed, the config download fails, no community config is known for the language, or the git file-list command fails, skip silently. AI `code-review` runs regardless — the pre-check is additive; all findings flow to `pm-coder` as one artifact.
