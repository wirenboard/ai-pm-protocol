...
**Make a context-read visibly distinct from a corner-cut (the transparency rider).** Reading an existing gate artifact for context before re-spawning the owning agent is legitimate — the `### Delegation & gate integrity rule` in `workflow/enforcement.md` permits the read; the ban is only on *presenting* the stale artifact as the gate result. But a legitimate context-read can still *read* to the PM as an attempted corner-cut. So when you read an existing gate artifact — an `audit-*.md`, a `_review.md`, a plan, any agent deliverable — for context before re-spawning the agent that owns it: (1) **announce the read-for-context-not-reuse intent BEFORE reading**, not after — name the artifact, say you are reading it for context only and not as the result, and that the gate needs a fresh run of the owning agent this turn; and (2) **do NOT relay the old artifact's findings or results to the PM** as if they were the current answer — the results the PM sees come from the FRESH agent's run, never the stale file. The transparency of the step is what distinguishes "reading for context" from "presenting the stale artifact": announced-after, or with the stale results relayed, it looks like a corner-cut even when the rule was followed.
> ✓ "I see a prior audit report on disk — reading it for context only, not as the result; the gate needs a fresh `pm-auditor` run this turn. [reads] Spawning `pm-auditor` now; I'll report its findings when it returns."
> ✗ "[reads] There's a prior audit here — 10 blocking, 6 notes. Let me re-run the audit to confirm." (announced after the read, and relayed the stale results as the answer)
...
This is the lighter discipline only — read-before-ship and rewrite-if-unclear. It is the **clarity** sibling of the **frugality** discipline in `## Durable-text frugality` in `workflow/doc-style.md`, which is the now-first-class home for the deeper comment-restraint rubric (`### Comment-restraint`: why-not-what, no trivial docstrings, no inline rule-IDs, single-home) and the structure rules (fact-first, current-state-only, supersede-don't-edit, provenance-as-pointer). The two never state the same rule — legibility asks "is the prose readable", frugality asks "is the structure lean"; read the frugality rule there by name, do not restate it here.
