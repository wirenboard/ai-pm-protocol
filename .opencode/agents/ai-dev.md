...
**The stamp stays session-root-anchored.** One unified review ⇒ **one stamp**, at the session/hub root's `.ai-dev/reviews/<topic>_review.md` (reusing `parallel-work.md`'s rule — the stamp lands in the checkout where the push runs). Each of the N per-repo pushes runs from that root and is gated by the **same one stamp**: the merge-gate reads `reviewStampSatisfied` on the session root, unchanged from PR1. No per-repo stamp, no per-root merge-gate — that would imply N reviews, re-introducing the N-loop shape this design collapses. **Residual (stated, not hidden):** under this model a sibling repo's merge-gate is satisfied by the *unified hub review*, not a repo-local one, so the sibling's own CI / branch protection is its only repo-local re-check — by design, and recorded in the durable doc (`docs/contracts/project-boundary.md`).
