...
- **Contracts regression** — if the project records product **contracts** (this repo: `docs/contracts/`; a downstream may use its own dir or none) and the change touches a behavioural guarantee, that guarantee's contract is re-checked and updated. A guarantee touched without its contract re-checked blocks. A NEW or CHANGED contracted surface — an MCP tool, HTTP route, event, schema, or public API — must land WITH its `docs/contracts/` entry (and the validating test where one exists). A contracted surface added or changed without its contract entry blocks.
