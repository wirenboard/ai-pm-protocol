...
- **The stamp also carries a `## Contracts:` heading, mandatorily** (8D `contracts-ignored-autonomous`, invariant 3's merge-gate reads it the same way as the verdict: any heading level, inline or next-line value). Its value is your **Contracts** checklist finding, in one line: the touched-contracts list and whether each held, or `none` when the change genuinely touches no contract. The verdict alone proves a review ran; this line proves the Contracts item was actually engaged, not silently skipped — an absent or empty `Contracts:` line blocks the ship exactly like an absent verdict, even when the verdict itself reads APPROVED.
