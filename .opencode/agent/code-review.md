...
# in the local config). True cross-model review (a different model than the
# session, for independent blind spots) is a separate deferred item that needs a
# second configured model + a PM pin; this engine runs SAME-MODEL for now.
