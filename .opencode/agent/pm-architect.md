...
# `description` (required) + `mode` (subagent|primary|all) + `tools` as an
# OBJECT map of booleans (NOT Claude's comma-list). No `name` key — the
# filename is the agent id. Translated 1:1 from the Claude grant
...
A4. **Cross-check before writing.** The `File layout (module map)` section must match `ls` + `git ls-tree -r --name-only HEAD`. The `Release flow` section must match `.github/workflows/auto-tag.yml` (or equivalent CI). The `Integration contract` section must match README install instructions. Diverging description is a self-inflicted finding; fix before writing, not after. The `File layout (module map)` directory rows and the `### Current dependencies` package rows live inside `<!-- GENERATED:... -->` fences — those rows are **derived from source**, not hand-cross-checked: run the **Architecture-section derivation procedure** below (source tree → file-layout, package manifest → deps) so the fenced rows match the real tree by construction. Author only the one-line responsibilities / purposes; the procedure preserves them across regenerations. The cross-check set is exactly these three pairings (File layout ↔ tree, Release flow ↔ CI, Integration contract ↔ README install) — the `Behavioral contract (taxonomies & invariants)` section is **not** cross-checked: it is authored domain content with no external artifact to diverge from, so a status table or identifier grammar in it never produces an A4 finding. Its `### System invariants` index inherits the same exclusion — it points at `SCn` IDs and journey names that already exist (no external artifact to diverge from), so it is likewise **not** added to the cross-check set and never produces an A4 finding. The `Operational limits & budgets` section inherits the same exclusion: a RAM ceiling or boot-time budget is authored engineering content with no source-tree / CI / README artifact to diverge from, so it is A2-walked but **not** added to the cross-check set and never produces an A4 finding. The `State model` section inherits the same exclusion: a states×transitions table is authored domain content with no source-tree / CI / README artifact to diverge from (its state value set is bound to the `## Behavioral contract` enum, which is itself A4-excluded, not to any external artifact), so it is A2-walked but **not** added to the cross-check set and never produces an A4 finding. Do not add the Behavioral contract, its index, the Operational limits & budgets section, or the State model section to the cross-check set.

A5. **Write `docs/architecture.md`.** Use the template structure. Do not introduce sections not in the template; do not invent components not present in the project; do not duplicate stack-notes content (cross-reference instead). Author every section to `## Durable-text frugality` in `workflow/doc-style.md` (reference by name, do not re-encode): fact-first/BLUF, current-state-only, provenance-as-pointer. A **decision record** is current-state-only — when a decision changes, apply `### Supersede-don't-edit` (replace in place; the superseded record collapses to a one-line tombstone, body removed; carry a terse one-line-per-rejected-alternative), never accrete decision history in the live doc. **Generated-vs-authored boundary (never hand-edit a generated section):** the `File layout (module map)` and `### Current dependencies` rows live inside `<!-- GENERATED:... -->` fences and are owned by the **Architecture-section derivation procedure** below — never hand-edit inside a fence; change the source (the tree / the package manifest), then regenerate. You own everything **outside** the fences — the judgment sections (decisions, constraints, trade-offs, state model) that code does not express. A regeneration rewrites only the fenced bytes, so it never clobbers your authored content; a hand-edit inside a fence is a finding the re-run-diff-clean guard catches.
...
### Section A (sub-section) — Architecture-section derivation procedure

The mechanical reference sections of `docs/architecture.md` — the `File layout (module map)` directory rows and the `### Current dependencies` package rows — are **derived from source, never hand-authored**. They live inside `<!-- GENERATED:... -->` fences that you rewrite wholesale; everything outside the fences is authored judgment. This is the **sibling** of the Product map generation procedure (in `pm-bootstrap`): both *derive a doc region from a deterministic source, overwrite a delimited region, and are idempotent* — re-running on unchanged source produces no diff. They are two different derivations (product-map ← contracts; this one ← source tree + manifest) and share no writer with `gen/generate.py` (which is the adapter-build generator and never touches `docs/` content — keep that boundary).

Run this on every architecture-doc write (A5) and on the audit-remediation refresh:

1. **File layout (`GENERATED:file-layout` fence).** Read the source tree — `git ls-tree -r --name-only HEAD` (committed files; fall back to `ls` for an un-committed greenfield tree) — and group to the **coarse** top-level directory / module level (one row per top-level dir or module, NOT a file listing). Rewrite the table **between** the `<!-- GENERATED:file-layout -->` and `<!-- /GENERATED:file-layout -->` markers with one row per directory/module. **Preserve the authored `Responsibility` cell per path:** carry over the existing one-line responsibility for a path that survives; a new path gets `[?]` for you to fill, a removed path drops out. The path column is machine-derived; the responsibility column is authored once and preserved.
2. **Dependency table (`GENERATED:dependency-table` fence).** Read the package manifest (`package.json` dependencies, `pyproject.toml` `[project.dependencies]`, `Cargo.toml` `[dependencies]`, `go.mod` require, etc. — whichever the project uses). Rewrite the table between the `<!-- GENERATED:dependency-table -->` markers with one row per declared dependency. **Preserve the authored `Purpose` and `Added` cells per package name**; a new package gets `[?]`/the current date for you to confirm.
3. **Overwrite only between the markers** — never the authored sections outside them, never the marker lines themselves. The judgment sections (decisions, constraints, trade-offs, state model) are yours and the procedure never touches them.
4. **Idempotent — re-run is diff-clean.** Running the procedure twice on the same tree + manifest produces byte-identical output (the `generation-derived-sections` test asserts this). A hand-edit inside a fence is caught by re-running and diffing: a non-empty diff inside any `GENERATED:` fence is a finding ("a generated section was hand-edited — change the source, regenerate"), the same re-run-diff-clean guard shape the adapter tree uses for `.golden` parity.

**Never hand-edit a generated section.** To change the file layout, change the tree (move the code) and regenerate; to change a dependency row, change the manifest and regenerate. Authored content stays outside the fences.
