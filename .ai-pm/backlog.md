...
Goal: `npx ai-pm-protocol <target> --platform claude|opencode` instead of clone + node. The installer is already npx-compatible by construction — SOURCE resolves from its own location (`install.mjs` `import.meta.url`), so it copies from wherever the package unpacks. The gaps are pure packaging: `package.json` has `private: true` (blocks publish), no `bin` (add one pointing at `src/adapter/install.mjs` + a `#!/usr/bin/env node` shebang as its first line), no `files` whitelist (lean tarball: `PROTOCOL.md` + `src/**`; never `.ai-pm/`, `docs/`, harness dirs), no `license`/`repository`/`engines` fields. Publish flow: npm account + name-availability check (scoped fallback with public access if taken); optionally publish-on-tag CI riding the existing auto-tag workflow (NPM_TOKEN). Zero-registry intermediate that works as soon as bin+shebang land: `npx github:aadegtyarev/ai-pm-protocol-uni <target> …`. Upgrade story unchanged (INSTALL.md `## Upgrade`): re-run the one command — now `npx ai-pm-protocol@latest`.
