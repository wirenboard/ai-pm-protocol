// Capability-module assembly test — proves the SHARED assembler (adapter/modules.mjs)
...
//
// Run: node adapter/install-modules.test.mjs
...
const reviewerFloor = fs.readFileSync(path.join(ROOT, "agents", "reviewer.md"), "utf8");
const orchFloor = fs.readFileSync(path.join(ROOT, "agents", "orchestrator.md"), "utf8");
...
console.log("SECURITY — missing fragment, root escape:");
{
  ...
  const badRegistry = { modules: [{ id: "ghost", fragments: { reviewer: "modules/ghost/reviewer.md" } }] };
  let threw = false;
...
check("in-root pointer resolves", resolveFragmentPath(ROOT, "modules/threat-model/reviewer.md").startsWith(ROOT));
