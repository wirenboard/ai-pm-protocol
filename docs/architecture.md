...
The boundary deny also widens — **read-only** — for the agent's OWN harness-assigned scratch, the out-of-root workspaces the platform itself places a role's artifacts in. Two roots, both read-allowed, both derived fail-closed from the platform's env:

1. **tool-result overflow store** — where a fetched doc / image whose body exceeded the inline cap spills its bytes (the agent must read those back, not be false-blocked as out of root).
2. **per-session harness temp root** — the platform's per-session temp dir (the scratchpad, staged pasted files, task outputs).

Both are the agent reading **its own** artifacts back — invariant 2's intent is "stay out of *other* projects", not "blind to the harness's own scratch". Without the widening the deny false-blocks a legitimate round-trip the platform relies on (the live symptom: an agent could not recognise a fetched image). The widening is **narrow**: a *sibling* session's transcripts stay denied (the temp entry is scoped to **this** session's id, not the shared `<slug>` parent), and `CLAUDE_CONFIG_DIR` broadly stays denied (only the narrow `…/tool-results/` subdir, never the config root or other projects' dirs).

**Derivation is fail-closed** (mirrors `componentRoots`): a path is admitted ONLY if its env is present, `fs.realpathSync` resolves it (the dir actually exists — a shifted platform convention ⇒ no widening, current strict behaviour, no crash), and it passes the overbroad guard (not a filesystem root, not an ancestor of / equal to the session root, not the shared parent that would leak a sibling session). Bad/absent env ⇒ empty allow-set ⇒ byte-identical to today; the two paths derive **independently** (one failing its check does not poison the other). The derivation lives in the **adapter** (`src/adapter/claude/sanctioned-scratch.mjs` — the platform's projects-dir slug + per-session temp-root conventions live there, never in the engine; the literal env vars and path shapes are in the adapter source + `docs/decisions/out-of-root-scratch-allow.md`), and is threaded onto the neutral `input.sanctionedScratch` by the shim. The engine's **read** predicates (`pathOutsideRoot`, `findTargetOutsideRoot`, `bashReadTargetOutsideRoot`) consult it via `isInsideSanctioned` in `engine-paths.mjs` **alongside** `isInsideAnyComponent`; the **write** predicate (`writeTargetOutsideRoot`) does NOT — the widening is read-only, a write outside the root stays denied. A bash `~/…` reference is expanded via `input.home` so it can reach a sanctioned path under `$HOME`. opencode's deny is a plugin; the same `input.sanctionedScratch` channel is the contract, opencode's own derivation (or none) is its adapter's job — Claude lands first, opencode parity is the documented follow-up (`docs/decisions/out-of-root-scratch-allow.md`). Read the derivation + the read/write asymmetry in `src/adapter/claude/sanctioned-scratch.mjs` and `engine.mjs`, and the guards in `src/adapter/claude/sanctioned-scratch.test.mjs`, not a restatement here.
