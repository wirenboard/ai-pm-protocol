...
**Platform filtering.** A single-platform project re-injects the role bodies every turn, so the *other* adapter's operating caveats are pure noise. `composeBody` takes the **assembling platform** (each shim passes its own literal) and, as its last step, strips any block delimited by own-line markers `<!-- platform:opencode -->…<!-- /platform:opencode -->` (or `platform:claude`) that is tagged for a *different* recognised adapter; the active platform's blocks are kept (markers stripped) and untagged/neutral content always survives (`filterPlatform`). Tag a passage `platform:X` only when it is guidance needed **solely** when *running on* X (e.g. the OpenCode `task`-runtime model-pin caveat) — a neutral contract or a comparative "on OpenCode A, on Claude B" passage stays untagged. **Fail-safe is KEEP**: an unknown/missing assembling platform does no filtering at all, and a typo'd tag (not a recognised adapter) is never dropped — content is dropped only on a *confident* other-platform match. **Scope (honest):** filtering reaches only *assembled* artifacts; a body brought in **raw** through a platform's load-instructions import (the Claude orchestrator, loaded as the session rather than assembled) does not pass through `composeBody` and so is **not yet filtered** (the runtime win for it needs the orchestrator to be assembled+filtered too).

A third role-content element sits beside the floor and the modules: **on-demand procedure bodies** — operating procedures too situational to ride every turn's context; the floor carries a one-line trigger, the body is read when the trigger fires. Not composed by the assembler — **read at run time** by the role. The source of truth is `src/agents/procedures/` (one home), but a downstream root has no bare `src/agents/` and the vendored copy lands under the unconditionally read-denied `.ai-dev/tooling/` (invariant 2) — so the installer **deploys** each procedure to a READ-allowed home, `.ai-dev/procedures/<x>.md`, the stable path the trigger names in both dogfood and downstream. That deployed copy is **committed + drift-guarded** (byte-identical to its source; `install-drift` row), exactly like the per-platform assembled agent artifacts — so the dogfood orchestrator always has it without a build step. Piloted by `parallel-work.md` (`docs/decisions/parallel-work.md`). The same deploy carries the one **runtime-read module file** — the elicitation catalog a role reads on-demand (vs the fragments the assembler bakes into agents) — to `.ai-dev/modules/<rel>` for the same reason and with the same committed drift guard (`deployModules` + `RUNTIME_READ_MODULE_FILES`, `install-core.mjs`).
...
- **Add a forge to the backlog adapter.** A row in `src/adapter/forge-map.json` (the forge's issue-CLI verbs for create/list/view/close/comment). This is the **backlog/forge axis**, orthogonal to the platform `tool-map.json`: it is **persona-read** reference data the orchestrator consults (`## Backlog`), **not** engine-loaded like `deny-rules.json`/`tool-map.json`, so it changes nothing mechanically; a shape test (`src/adapter/forge-map.test.mjs`) guards it from rotting. No core edit.
- **List available models.** A neutral contract point setup uses to put real model choices before the Operator (the orchestrator's `## Setup`). Each adapter realises it: Claude returns its known allow-listed model set, OpenCode discovers the environment's models — realisation noted in `tool-map.json` `models`, no environment-specific id in the core.
...
An entry MAY also carry an optional **`contracts`** field — an array naming the seam contracts that component **owns** and who consumes them: `"contracts": [ { "path": "docs/contracts/api.md", "consumers": ["frontend"] } ]`. This is **advisory metadata the orchestrator reads** to coordinate a cross-component feature (which sibling owns a seam, who depends on it); the seam-contract transport design is `docs/decisions/seam-contract-transport.md`, the in-session-transport posture is in the boundary contract (`docs/contracts/project-boundary.md`), and the orchestrator's use of it is in `src/agents/orchestrator.md` `## Multi-component coordination`. **The security-critical validator (`componentRoots`) IGNORES `contracts` entirely** — it reads only `root` for the boundary set and inspects no other key, so an advisory field can never widen the boundary (pinned by `src/adapter/components.test.mjs`: a `contracts`-bearing manifest still widens correctly, and a `contracts`-bearing entry with a bad `root` still fails CLOSED to the single session root). The field stays additive: an entry without it is unchanged.
...
The boundary deny **consults this set**. The three project-boundary predicates (`pathOutsideRoot`, `findTargetOutsideRoot`, `writeTargetOutsideRoot`) test membership through `isInsideAnyComponent` in `engine.mjs` — a target is inside the boundary iff it is inside *any* validated declared root. With no/invalid manifest `componentRoots` returns just the session root, so this is byte-identical to the single-root `isInsideRoot` — the no-manifest regression tripwire. The `.ai-dev/tooling/` self-patch guard also consults the set, but to **narrow** rather than widen: `writesIntoAnyNever` denies `.ai-dev/tooling/` under *every* declared root, so a sibling's enforcer source is covered too and a manifest can never open a path into a tooling dir (the `sibling-tooling` parity cases prove it). Every **other** deny — truncation, the stamp-write guard, the orchestrator-content guard, the merge-gate — does **not** consult the set at all: it stays anchored to the session root via `isInsideRoot`, unconditional. Read the predicates and their callers in `engine.mjs`, not a restatement here.

## Sanctioned scratch

The boundary deny also widens for the agent's OWN harness-assigned scratch, the out-of-root workspaces the platform itself places a role's artifacts in. Two roots, both derived fail-closed from the platform's env, with **different read/write scope**:

1. **tool-result overflow store** — where a fetched doc / image whose body exceeded the inline cap spills its bytes (the agent must read those back, not be false-blocked as out of root). **Read-only** — the harness's own spill artifact, never the agent's to write.
2. **per-session harness temp root** — the platform's per-session temp dir (the scratchpad, staged pasted files, task outputs). **Read AND write** — the harness's own system prompt directs the agent to write ephemeral temp files here, so this is the sole write-allowed target outside the session root.

Both are the agent touching **its own** artifacts — invariant 2's intent is "stay out of *other* projects", not "blind to (or unable to use) the harness's own scratch". Without the read widening the deny false-blocks a legitimate round-trip the platform relies on (the live symptom: an agent could not recognise a fetched image). Without the write widening, a compliant agent following the harness's own directive to write temp files there is false-blocked doing so (the live symptom motivating the write extension, `#401`). The widening is **narrow** either way: a *sibling* session's temp files/transcripts stay denied for both read and write (the temp entry is scoped to **this** session's id, not the shared `<slug>` parent), and `CLAUDE_CONFIG_DIR` broadly stays denied (only the narrow `…/tool-results/` subdir, never the config root or other projects' dirs).

**Derivation is fail-closed** (mirrors `componentRoots`): a path is admitted ONLY if its env is present, `fs.realpathSync` resolves it (the dir actually exists — a shifted platform convention ⇒ no widening, current strict behaviour, no crash), and it passes the overbroad guard (not a filesystem root, not an ancestor of / equal to the session root, not the shared parent that would leak a sibling session). Bad/absent env ⇒ empty allow-set(s) ⇒ byte-identical to today; the two paths derive **independently** (one failing its check does not poison the other). The derivation lives in the **adapter** (`src/adapter/claude/sanctioned-scratch.mjs` — the platform's projects-dir slug + per-session temp-root conventions live there, never in the engine; the literal env vars and path shapes are in the adapter source + `docs/decisions/out-of-root-scratch-allow.md`) as a single **tagged** computation (`{path, writable}`), computed once: `deriveSanctionedScratch` returns all paths (read), `deriveSanctionedScratchWritable` returns only the writable one (write) — no derivation logic duplicated between them. Both are threaded onto the neutral input by the shim as `input.sanctionedScratch` and `input.sanctionedScratchWritable`. The engine's **read** predicates (`pathOutsideRoot`, `findTargetOutsideRoot`, `bashReadTargetOutsideRoot`) consult `input.sanctionedScratch` via `isInsideSanctioned` in `engine-paths.mjs` **alongside** `isInsideAnyComponent`; the **write** predicate (`writeTargetOutsideRoot`) consults the SAME `isInsideSanctioned` helper but with `input.sanctionedScratchWritable` — the narrower set, so a write to the overflow store still denies while a write to the temp root allows. A bash `~/…` reference is expanded via `input.home` so it can reach a sanctioned path under `$HOME`. opencode's deny is a plugin; the same `input.sanctionedScratch`/`input.sanctionedScratchWritable` channels are the contract, opencode's own derivation (or none) is its adapter's job — Claude lands first, opencode parity is the documented follow-up (`docs/decisions/out-of-root-scratch-allow.md`). Read the derivation + the read/write scoping in `src/adapter/claude/sanctioned-scratch.mjs` and `engine.mjs`, and the guards in `src/adapter/claude/sanctioned-scratch.test.mjs`, not a restatement here.
