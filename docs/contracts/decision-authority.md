...
- Merge and ship need the PM's explicit authorization in both modes — autonomy carries a feature to a finished, reviewed result and stops there; nothing merges without the PM's per-merge authorization (never inferred), though the loop may *execute* the merge once it is given.
