...
- Every agent stays within the project root — never reads, searches, or writes outside it; the carve-out inside the root is that the enforcer's own source (`.ai-dev/tooling/`) is off-limits to read and write.
