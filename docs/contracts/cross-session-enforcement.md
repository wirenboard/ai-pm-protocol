...
- The guards hold without the session having re-read the rules, and are themselves tested so a regression fails CI — the merge-gate's stamp forms and edge cases are pinned by `src/adapter/merge-gate-*.test.mjs` (stamp-resolution · ref-parsing · trunk-and-worktree).
