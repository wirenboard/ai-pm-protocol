...
> **This section's own DIALOG SHAPE (the fully-per-seat menu + its free-the-alias auto-allocation) is in turn superseded by papercut 14 (synonym-first two phases).** The **mechanic below is unchanged and load-bearing** — read this section for the bare-alias-vs-concrete-id resolution and the latent-defect fix; read papercut 14 for the shipped dialog.
...
### Papercut 14 — synonym-first dialog + a 4th (`fable`) tier + independent launch seats + the planner seat (the Operator's decisions, live `/dev-setup` dogfood, 2026-07-01)

**Supersession.** This section **supersedes the operator-facing DIALOG SHAPE of papercut 13** — the fully-per-seat menu (each seat asked native-Claude-OR-a-foreign-provider independently) and the auto-allocation it drove (distinct-tier assignment + the "free-the-alias" concrete pin). The underlying **mechanic papercut 13 established is unchanged and load-bearing**: a baked seat rides a **bare synonym alias** (routes foreign via `ANTHROPIC_DEFAULT_<SYNONYM>_MODEL`) when its synonym is bound foreign, a **concrete id** (native) otherwise; `resolveModelPin` + `boundTiers` + `verifyRoutingConsistency` are unchanged in structure. Only the way the Operator is *asked*, the **number of tiers (3 → 4)**, and the **launch-seat independence** change. (Invariant 6: supersede, don't accumulate — papercut 13 remains the mechanic + latent-defect record; the dialog it describes is no longer shipped.)

**Grounding (official Claude Code docs, via the claude-code-guide research, confidence "Certain", 2026-07-01).**

- There are **FOUR remappable tier aliases**, not three: `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU,FABLE}_MODEL`. So `fable` is a first-class remappable synonym (Claude's top "hardest / longest-running" tier; the `best` selector = Fable-or-Opus).
- `best` / `opusplan` are **NOT** independently remappable (convenience selectors) — excluded from the synonym set.
- **Custom arbitrary aliases are not supported** — the set of four is the fixed ceiling (CC issues #34821 / #12386 / #14443 closed not-planned).
- **`ANTHROPIC_SMALL_FAST_MODEL` is deprecated** in favour of `ANTHROPIC_DEFAULT_HAIKU_MODEL`, still honoured, no removal date.
- The **guard/background is COUPLED to the haiku slot** in the modern path (`ANTHROPIC_DEFAULT_HAIKU_MODEL` = "haiku, or background functionality"); the ONLY knob that sets the background independently of the haiku slot is the deprecated `ANTHROPIC_SMALL_FAST_MODEL` — this is exactly decision **G1**.

**The defects the dialog carried (two rounds).** (1) papercut 13's fully-per-seat menu rendered live as a buggy option set — each seat's native choice **truncated** (reviewer offered only `opus`) and the `foreign` choice **mixed into the seat pick**, collapsing two orthogonal questions onto one menu. (2) The first synonym-first cut over-corrected: it made **session and guard follow their synonym's global mapping**, which **removed the ability to keep a launch seat native while a baked seat routes that tier foreign** — a regression, since the installer already supports the independent launch-env path. The live dry-run also wanted **`fable` placeable on any seat** and the **planner** exposed in the dialog.

**The Operator's decisions (ratified, structured-question, 2026-07-01).**

- **Synonym-first, phased.** The dialog splits the orthogonal questions: **Phase 1 — BAKED seat → synonym** (planner · builder · reviewer each pick `fable`/`opus`/`sonnet`/`haiku` — VISIBLE, strongest→weakest); **Phase 2 — synonym → model** (each of the four maps native or foreign; **all four mapped, even an unridden one**, so a later reseat needs no re-bind).
- **`fable` as the 4th synonym**, usable on any seat. Added the sanctioned "new model = data, not code" way: `fable` in `tool-map.json` `models.claude.allow` + `"fable": "claude-fable-5"` in `ids`; `aliasOf` already loops the whole allow-list, so `resolveModelPin`/`boundTiers` pick it up for free. The one env-map edits: `install-claude.mjs` `LAUNCH_ALIAS_ENV_KEYS` and its launcher-side mirror `router-launch.mjs` `launchModelEnv` `ALIAS_ENV` both gain `fable → ANTHROPIC_DEFAULT_FABLE_MODEL` (they MUST stay in sync — the installer writes `settings.json` env, the launcher exports it at startup), and `isVanilla` loops four tiers. Order everywhere strongest→weakest: `fable · opus · sonnet · haiku`.
- **Launch seats chosen DIRECTLY** (session · guard), not by a synonym's global mapping — restoring independence. **Session:** picks a concrete model via `ANTHROPIC_MODEL` (any native tier incl. `fable`, or a foreign id), **native even over a foreign-bound synonym** (a concrete id bypasses the alias — the papercut-13 free-the-alias path, taken by construction; `ANTHROPIC_MODEL` is not deprecated). **Guard:** **default = the haiku slot** (`ANTHROPIC_DEFAULT_HAIKU_MODEL`, modern, no risk — leave `guardModel` empty); an **override** to a background different from the haiku slot uses the deprecated `ANTHROPIC_SMALL_FAST_MODEL` (G1) with a plain deprecation warning. **Bonus a 4th tier unlocks:** route a cheap foreign baked seat via a NON-haiku tier and leave haiku native → the background stays native on the modern var, no override needed.
- **Planner in the dialog.** The installer **already bakes** `roles.planner.model` (it is in `SPAWNABLE` alongside builder/reviewer — confirmed, docs-only change), so the dialog just adds planner to the Phase-1 baked-seat list with an explicit **"inherit session (default — a strong planner)"** option, preserving the cheap-builder / strong-session-planner economy (`docs/decisions/planning-model-split.md`).

**The scope of "globally native or foreign" — BAKED seats only.** Under the four-tier design a synonym is globally native or foreign **for the baked seats** (planner/builder/reviewer) that ride it as a bare alias. The **launch seats are independent**: the session is a direct concrete id (native even over a foreign-bound tier — that IS the free-the-alias path, now always taken by construction, so no conditional re-pin exists); the guard defaults to the haiku slot with a deprecated override. This is the correction over the first synonym-first cut, which wrongly coupled session/guard onto their synonym's global mapping.

**The config SHAPE is a superset of papercut 13's — no breaking change.** `roles.{planner,builder,reviewer}.model` = the synonym; `launch.aliases.<synonym>` = the foreign id (four tiers now; personal `config.local.json`, or shared `config.json` for a team binding); `launch.sessionModel` = the session's chosen concrete id; `launch.guardModel` = empty (haiku-slot default) or the deprecated-var override. An unused synonym mapped foreign lands in `launch.aliases`; `mergeLaunchEnv` writes its `ANTHROPIC_DEFAULT_<SYNONYM>_MODEL` and `verifyRoutingConsistency`'s bound-synonym loop validates the env landed — it never requires a consuming seat — so a stable-but-unridden binding is consistent, not a false red.

**Worked example — the exact config the allocation produces.** Phase 1 planner=inherit-session · builder=sonnet · reviewer=fable; Phase 2 fable=native · sonnet=foreign GLM (opus/haiku native); launch session=opus (native), guard=default:
...
```json
{
  "launch": {
    "sessionModel": "claude-opus-4-8",
    "guardModel": "",
    "aliases": { "sonnet": "glm-4.6" }
  },
  "roles": {
    "orchestrator": { "agent": "ai-dev" },
    "planner":  { "agent": "dev-planner" },
    "builder":  { "agent": "dev-builder",  "model": "sonnet" },
    "reviewer": { "agent": "dev-reviewer", "model": "fable" }
  }
}
```

Resolution trace: **builder** — assembled `model: sonnet` (bare, sonnet bound) → `ANTHROPIC_DEFAULT_SONNET_MODEL=glm-4.6` → GLM. **reviewer** — `model: claude-fable-5` (concrete, fable unbound) → native fable. **planner** — no `model:` line → inherits the session model (strong planner). **session** — `ANTHROPIC_MODEL=claude-opus-4-8` (concrete → opus native, no alias). **guard** — `guardModel` empty ⇒ `ANTHROPIC_SMALL_FAST_MODEL` unset ⇒ background follows the native haiku slot; builder routed foreign via the non-haiku sonnet tier keeps it native (the 4th-tier bonus, no deprecated override).

**The one residual (unchanged from papercut 13).** The subagent-frontmatter × bare-alias intersection — that a baked `model: sonnet` on a *subagent* resolves through `ANTHROPIC_DEFAULT_SONNET_MODEL` exactly as the session model does — is still the one live rung, offered as a real-layer verification (the static self-verify catches the config↔artifact class at apply time, not Claude's runtime resolution).
