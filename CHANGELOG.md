...
- **PATCH** — document the cwd re-anchoring hazard for a spawned/continued agent (#404). The "Git ops on a nested or sibling repo" bullet (`orchestrator.md ## Your seat`) already forbade `cd <path> && git …` for git-targeting reasons, but said nothing about the wider consequence: a leftover `cd` re-anchors the sandbox root of **every** subsequent tool call — a spawned or continued agent's own boundary, a read, a write, not just a later git command's target — because the boundary resolves to the nearest ancestor carrying `.ai-dev/config.json`, which a worktree copies. Observed live in a parallel-worktree session: a stray `cd <worktree> && git …` left the shell rooted there, and a later Reviewer spawn intended for the main checkout silently resolved into that worktree instead — caught safely (the mismatched agent BLOCKED rather than proceeding), but at the cost of a wasted round-trip. Fix is documentation-only, matching the thin-core call recorded in the plan: extend the one authored bullet with the consequence plus the discipline (use `git -C` exclusively, never leave the shell `cd`'d into a subdirectory), and add a pointer from `parallel-work.md`. No enforcer change — the boundary already fails closed; this removes the foot-gun that triggers it. PATCH — a role-prose clarification, no new agent/command/module, no contracted surface change.

## [5.64.2] — 2026-07-08

- **PATCH** — extend the sanctioned-scratch carve-out to **writes**, closing a self-inflicted friction where the boundary denied the very writes the harness system prompt tells the agent to make (#401). The Claude harness instructs the agent to put all temp files in its per-session scratchpad (`<TMPDIR>/claude-<uid>/<slug>/<sid>/…`), outside the project root — but the boundary deny admitted that scratch for **reads only** (5.62.0), so a compliant temp write was blocked by our own enforcer (observed live; the model then invents workarounds — reaching for a sibling repo, `cd`-ing out — the same secondary-confusion pattern as #400). Fix: a **narrow, fail-closed write carve-out** for the agent's OWN per-session temp root, and nothing more. `deriveSanctionedScratch` (reads — output byte-identical) and a new `deriveSanctionedScratchWritable` (writes — the temp root only) share one internal `deriveTagged`; the tool-result **overflow store** is tagged non-writable and can never enter the write set (it is the harness's, read-only forever). `writeTargetOutsideRoot` gains exactly one clause (`&& !isInsideSanctioned(input.sanctionedScratchWritable, r)`); the read predicates are untouched. **Floor properties preserved and adversarially tested:** fail-closed (absent/blank env ⇒ empty writable set ⇒ byte-identical to the prior DENY), session-id-narrow (a sibling session's `<sid>` stays write-denied; a `sid="../.."` escape ⇒ `[]` via the existing `admissible` overbroad guard), overflow-store write still denied, no other deny (self-patch/tooling, merge-gate, stamp-write, component boundary) relaxed. Invariant 2's "writes outside the root stay denied" is narrowed to name the temp-root exception, mirrored one-home across `PROTOCOL.md`, `protocol-reference.md`, `docs/architecture.md`, `docs/decisions/out-of-root-scratch-allow.md`, and the re-baked agents. New unit + real-`shim.mjs`-subprocess E2E assertions (RED without the engine clause, GREEN with it). Claude-only (OpenCode has no scratch env); engine stays neutral. PATCH — a deny-layer false-denial fix, no new agent/command/module/template.

## [5.64.1] — 2026-07-08

- **PATCH** — fix a dogfood-blind runner-path split that blocked a fresh downstream on its first `build` beat (#400). The installer deploys the quality runner to `.ai-dev/quality/run.mjs` (`install-core.mjs`), and the pre-push hook, `audit.md`, and `setup.md` all invoke it there — but the assembled Builder/Reviewer agent bodies and the template `tools.json` `_doc` still named the **source-repo** path `src/quality/run.mjs`, which a downstream install never lays down (no `src/` tree). A downstream Builder therefore read `node src/quality/run.mjs build`, found nothing, and declared the gate blocked (reported live via OpenCode; the session then invented workarounds — reaching for a sibling protocol clone's `package.json`, `cd`-ing out — secondary confusion on top of the real bug). Invisible to CI/dogfood because the source repo genuinely carries `src/quality/run.mjs` (dogfood skips `layDownCore`), so the dogfood Builder found it and stayed green — the same class as the 5.60.1 pack regression (dogfood SOURCE path ≠ downstream installed path). Fix: **canonicalise on the single runtime path `.ai-dev/quality/run.mjs`** in `builder.md`, `reviewer.md`, and the template `tools.json` `_doc`; the dogfood install branch now also deploys the runner to `.ai-dev/quality/run.mjs` (a single `copyFile`, gitignored via `.ai-dev/quality/` — a build artifact, never a committed duplicate of `src/quality/run.mjs`, invariant 1), so the one string is correct in both worlds without a per-mode assembly rewrite. **New regression tripwire** (`install-core.test.mjs`): a real non-dogfood install extracts the runner path the assembled Builder/Reviewer/fixup bodies name and asserts it both exists in the installed target and equals `.ai-dev/quality/run.mjs` — RED before this fix on two independent assertions, GREEN after; the dogfood-only path check that missed the class now has its downstream counterpart. PATCH — a downstream-blocking bug fix, no engine/deny change, no contracted surface change.

## [5.64.0] — 2026-07-08

- **MINOR** — a PostToolUse checkpoint-nudge hook: the mechanical backstop for the orchestrator's continuous crash-resume checkpoint rule (#397). That rule (`orchestrator.md ## Your seat` — refresh the active plan's progress note after each significant step) is `[persona]` only, so it drifts without a reminder (observed live across a long session). This adds its backstop, a sibling to the SessionStart pointer-inject (5.56.0) and the `pointer-size` cap (5.62.0): a **PostToolUse hook scoped to the `Task` tool** (a spawned-role handoff — the canonical significant step) makes `claude/shim.mjs` emit a **pure inject** nudging the orchestrator to refresh the active plan's progress note (supersede, not append) and reconcile the pointer. The PostToolUse branch sits before the engine `decide()` and returns directly — it never runs a Task through the deny path; it only fires for `Task`, and no other hook event is hijacked. **Inject-class — nudges, never blocks; no engine/deny change.** Realised mechanically on Claude (`[mechanical]`), `[persona]` on OpenCode (its inject hook was dropped in 5.17.7 — M18). The orchestrator's checkpoint rule now names this hook as its Claude backstop while keeping its `[persona]` label — the inject reinforces the act (delivery mechanical), never replaces it. MINOR — a new inject hook, backward-compatible; no contracted surface change.

## [5.63.0] — 2026-07-08

- **MINOR** — adopt the Claude `fork` subagent for the orchestrator's own direct work (#376; research: `docs/decisions/fork-primitive-adoption.md`, "adopt with guards"). When the Orchestrator plans or builds **directly** (the profiles that don't staff a role — `solo`/`lite`-simple/`yolo`), it MAY now fork itself so that work runs in a child that inherits the full context, returns only the result, and keeps its tool-output out of the main session — cleaner resident context, fewer resets. **Zero enforcement risk / zero floor change:** on Claude the shim leaves `isOrchestrator` undefined and both actor-dependent denies already short-circuit, so a fork adds nothing to gate; `engine.mjs`, `deny-rules.json`, and `claude/shim.mjs` are untouched. Realised as one **optional** `tool-map.json` capability point `fork-for-direct-work` (`claude: Task(subagent_type='fork')`, `opencode: null`, `_fallback: in-session-direct`), a sibling to `continue-a-sub-agent`; a platform with no fork primitive (OpenCode) or a wrapper-less launch falls back to in-session-direct with **no capability lost**. **HARD guard, stated in both the tool-map `_doc` and the orchestrator `## Your seat` prose:** NEVER fork for the **Reviewer** seat (a fork inherits the parent context and cannot be the independent reviewer invariant 3 requires — always a cold, non-inheriting fresh spawn) nor a full-profile **Planner** (its adversary value needs a fresh, solution-blind context). The fork self-checkpoints to the active plan's progress note. `[persona]` — no deny enforces which primitive the Orchestrator uses; a `parity.test.mjs` case pins that a `fork` spawn is not in the deny set. MINOR — a new optional adapter capability + orchestrator guidance, backward-compatible; no contracted surface change.
...
- **MINOR** — an in-project home for durable cross-session memory + thin-pointer enforcement (#316). Durable project knowledge (learned gotchas, "why we do X here", working context that outlives a session) had no in-project home, so it leaked into the resume pointer (the journal anti-pattern — invariant 6; `.ai-dev/state/current.md` bloated across this campaign) or into the external per-profile harness memory (unshared, invisible to the boundary). This gives it a home and makes the thin pointer mechanical:
  - **`.ai-dev/notes/`** — a **committed**, team-shared durable-memory home (seed `README.md` states the taxonomy); `install-core.mjs layDownCore()` seeds it downstream (idempotent, absent-guarded).
  - **Knowledge taxonomy** (`PROTOCOL.md` beat 1 + `orchestrator.md ## Your seat`): **checkpoint** (volatile working state — the pointer + the plan progress note) ≠ **durable notes** (`.ai-dev/notes/`, committed) ≠ **formal decisions** (`docs/decisions/`) ≠ **external scratch** (temp trash, stays external). The pointer is SUPERSEDED each update, never appended.
  - **Host-memory routing rule + trigger** (`[persona]`): durable knowledge → `.ai-dev/notes/`, **never** the harness `#`/host memory (external, per-profile, unshared), never the pointer, never external scratch — with a trigger at the moment the model would reach for the memory tool. No deny forces it (honest).
  - The installer sets **`autoMemoryEnabled: false`** so the model's reflexive auto-memory never writes to the external per-profile store (an Operator-approved substitution for the launcher `autoMemoryDirectory` divert, which proved infeasible — the setting is settings.json-only, has no env form, and an absolute in-repo path is not portably committable).
  - **`pointer-size` build check** (cap 120 lines / 8192 chars) — RED when `.ai-dev/state/current.md` bloats, fail-safe on an absent pointer; mirrors `agent-size`. The mechanical backstop the persona "keep the pointer thin" rule lacked (proven necessary: the orchestrator drifted this very session while knowing the rule).
  - **#365 deferred** honestly: the attached-artifact image-cache uses the default `~/.claude/`, not `CLAUDE_CONFIG_DIR`, so it is not env-derivable fail-closed and the read-block could not be confirmed — no speculative widening of the `[mechanical]` boundary floor (`sanctioned-scratch.mjs` untouched).
  MINOR — a new committed doc home + installer default + build check, backward-compatible; no engine/deny change, no contracted surface change.

## [5.61.1] — 2026-07-08

- **PATCH** — the commit/merge gate resolves its target repo from `git -C <path>`, not the session root (#383, M3). The `commitOnUnstampedMain` and merge-gate push-`main` denies resolved which repo they guard from `payload.cwd` (the shell cwd = session root), never the command's `-C` argument — so a legitimate `git -C /other-repo commit` or `git -C /other-repo push origin main` was **falsely denied** whenever the session's own repo sat on `main`/unstamped (reproduced live: a downstream-install-test commit blocked because this repo is on `main`). `#298` fixed the read/write/find nested-repo denies; this is the commit/merge-gate analogue that was missed. Fix: new `extractGitEffectiveCwd(command, cwd)` in `src/adapter/session-root.mjs` pulls the single `-C <path>`, `path.resolve` + `realpathSync` it (**fail-CLOSED**: zero / multiple / missing-path / non-string all return the original `cwd`), and `claude/shim.mjs` feeds that to `targetsSessionRepo`. Engine byte-unchanged (it already short-circuits on `targetsSessionRepo === false`). **Floor intact** (meticulous-reviewed): a bare `git commit` / `git push origin main` in the session repo while on `main`/unstamped stays DENIED; only a provably-different `-C` repo is exempt; a symlink resolving back to the session repo still denies. Adds an orchestrator `[persona]` convention: a nested/sibling-repo git op uses `git -C <path>`, never `cd <path> && git` (the `cd &&` form stays session-judged by design — parsing it would add fail-open surface to a security deny). 18 new nested-repo-scope tests (unit + real-shim E2E). PATCH — a false-denial bug fix in the deny layer, floor-preserving; no contracted surface change. OpenCode: unchanged (it doesn't compute `targetsSessionRepo`; the deny stays fail-closed there).

## [5.61.0] — 2026-07-08

- **MINOR** — floor-only Reviewer body for fixup-grade spawns (#387 / M14, token economy). The baked role agents carry the full capability-module stack on every spawn — `dev-reviewer.md` is ~36k chars (~9k tok) — but on a **fixup-grade** change (a typo, a one-line fix) the module depth-adders (threat-model, ui-ux, performance, …) buy nothing the change needs, while the fixup Reviewer pass is already "shortened, never skipped." New `composeFloorOnly()` in `src/adapter/modules.mjs` bakes a **`dev-reviewer-fixup.md`** variant (both platforms) = the Reviewer body with capability modules stripped but §Invariants **and the full `## Check` floor kept**. The fixup body is an **exact ordered subsequence** of the full body — every gate (plan-conformance, security, correctness, honesty/over-claim, contracts, tests) retained verbatim; only the 138 lines of module depth dropped: **36,214 → 21,226 chars (~3.7k tok saved per fixup Reviewer spawn)**. The Orchestrator spawns `{agent}-fixup` (derived from `roles.reviewer.agent`, no `config.json` change) for fixup-grade Reviewer passes, **failing safe to the full body when uncertain** (`## Fixup` / `## Your seat`, `[persona]` — no deny enforces which body spawns). Reviewer-only (the Builder has no modules marker — already floor-only; the Planner is out of scope). Claude bakes the same model pin as the full Reviewer; OpenCode bakes the variant too (live plugin-load is the standing M8 caveat). Tests: new `src/adapter/install-fixup-agents.test.mjs` + `invariants-parity` includes the variant. MINOR — a new installer capability (a spawn-body variant), backward-compatible; no contracted surface change.
...
- **PATCH** — fix a downstream install regression from 5.60.0: `protocol-reference.md` was missing from the published package. 5.60.0 (#369) taught the installer to `copyFile` the new companion `protocol-reference.md` from the package root, but the file was not added to `package.json` `files`, so npm/git packed the tarball WITHOUT it — a downstream `npx github:wirenboard/ai-pm-protocol` (or npm install) then failed with `ENOENT … copyfile … protocol-reference.md`. Fix: add `protocol-reference.md` to `files`. **Root-cause of the miss:** the `install-drift` tests run in dogfood mode (SOURCE === target, where the file is always present at the repo root), so they never exercised the packed tarball's `files` whitelist. New guard `src/adapter/pack-completeness.test.mjs` (build beat) extracts every root-level file the installer copies from SOURCE and asserts each is in `npm pack --dry-run`'s published list — proven to bite (fails when `protocol-reference.md` is dropped from `files`). PATCH — a shipped-regression fix, no contracted surface change.

---

## [5.60.0] — 2026-07-07

- **MINOR** — structural split of `PROTOCOL.md`: the three reference-only sections move to an on-demand companion doc (#369, token-optimization campaign lever C). The orchestrator resident-loads the whole constitution every turn; on small-window models each compaction re-pays it. `## Enforcement` (deny-rule catalog — the shim enforces it from `deny-rules.json` regardless of the prose), `## Project config` (field-by-field semantics), and `## Core and adapter` (adapter-contract rationale) move **verbatim** into a new companion **`protocol-reference.md`**, each replaced in `PROTOCOL.md` by a short resident summary + pointer. Resident `PROTOCOL.md` shrinks ~14.8k chars (~3.5k tokens), re-paid on every compaction cycle. The companion is a **sibling of `PROTOCOL.md`**: source `protocol-reference.md` at the repo root, deployed downstream to `.ai-dev/protocol-reference.md` (the installer mirrors `PROTOCOL.md`'s own tooling + `.ai-dev/` copies in `install-core.mjs`), cited as the bare sibling name so the pointer resolves in both dogfood and downstream layouts. **Pure move — no rule's content or scope changed** (each moved section byte-verbatim bar two named in-file cross-ref adaptations; independently confirmed by a meticulous diff review). All ~29 cross-references across source/docs repointed (zero dangling pointers). **Feature-A parity intact:** `## Invariants` stays resident and remains immediately followed by `## Role contracts`, so the composed-into-agents extraction bound is unchanged; `invariants-parity` + `install-drift` green. **Downstream fail-safe:** a missing companion (old install) means the pointer simply doesn't resolve — no rule change, no crash; `src/adapter/upgrades.md` tells existing installs to re-run the installer to receive it. Reverses the ratified lever-3 descope in `docs/decisions/protocol-surface-optimization.md` (Operator-authorised 2026-07-07 — the 200k-window compaction pain meets that doc's "revisit only if more window budget is genuinely needed" condition). MINOR — a structural re-home of core protocol text, backward-compatible (no contracted surface change; no MCP tool / HTTP route / event / schema / public API).

---

## [5.59.0] — 2026-07-07

- **MINOR** — the Claude adapter denies the built-in `Workflow` tool by default (#368, token-optimization campaign lever D). Claude Code ships the `Workflow` tool's full JSON schema resident in the system prompt (~5k tokens) whether or not it is used; the protocol orchestrates through the `Agent` tool and **never** references `Workflow` (every source hit is the legacy `WORKFLOW.md` filename, the `wb-git:workflow` skill, or a CI reference — no tool invocation, and no assembled agent's `tools:` frontmatter grants it). `wireClaude` (`src/adapter/install-claude.mjs`) now writes `"permissions": { "deny": ["Workflow"] }` into the installed `.claude/settings.json`; a **bare** tool deny drops the schema from context entirely (not just blocks invocation), reclaiming ~5k resident for every downstream Claude project — re-paid per compaction cycle on small-window models. The merge is **idempotent and non-clobbering**: it preserves a user's own `permissions` object and any pre-existing `deny`/`allow` entries and appends `Workflow` only if absent; it never denies `Agent`/`Task` or the dev tools (Read/Edit/Write/Bash/Grep/Glob). The dogfood `.claude/settings.json` is re-baked to match. Test coverage in `src/adapter/install-claude-wiring.test.mjs` (default-deny present · not-Agent/dev · pre-existing deny+allow preserved · idempotent on re-install · a coherence guard that no assembled agent grants the tool). Claude-only (opencode has no `Workflow` tool). MINOR — a new adapter install default, backward-compatible (no contracted surface change; no MCP tool / HTTP route / event / schema / public API).

---

## [5.58.0] — 2026-07-07

- **MINOR** — compose `PROTOCOL.md ## Invariants` into the spawned role agents at install time (#367, token-optimization campaign lever A). Each spawned Builder / Reviewer / Planner previously re-read the whole `PROTOCOL.md` (~10k) on every spawn just to carry the invariants that bind it. The invariants now ride in the agent's own composed prompt: a new own-line marker `<!-- ai-dev:invariants -->` in each source agent (`src/agents/{builder,planner,reviewer}.md`) is replaced at assembly with the extracted `## Invariants` block (heading through the line before the next `##` heading), mirroring the existing `ai-dev:modules` marker mechanism in `src/adapter/modules.mjs` (same regex-escape + trailing-newline handling + spacing collapse). The binding line in each source agent now points at the composed block and keeps an escape hatch — read the full `PROTOCOL.md` only when the task needs the loop or enforcement detail; `PROTOCOL.md` line 3 is softened to match (the Orchestrator still reads it whole; a spawned role carries the invariants and reads the rest on demand). **One-home preserved:** `PROTOCOL.md` is the source, the installer copies, and a new parity test (`src/adapter/invariants-parity.test.mjs`, registered in the `build` beat of `src/quality/tools.json`) asserts every committed assembled Claude role agent carries the `## Invariants` block **byte-identical** to `PROTOCOL.md` — a `PROTOCOL.md` §Invariants edit without a re-bake fails the build (guard proven to bite: mutate → fail, revert → pass). Composed into all six assembled agents (`.claude` + `.opencode`); cross-platform one-home also covered by `install-drift.test.mjs`. Net per spawn: −10k read, +~1.6k composed block (6395 B) ≈ **−8.4k/spawn** (~17k/feature on `solo`). MINOR — a new install-time assembly mechanism, backward-compatible (no contracted surface change; no MCP tool / HTTP route / event / schema / public API). No change to the module registry / capability-module axis (a sibling marker, not a new module); the structural split of `PROTOCOL.md` (lever C) and the density pass (lever B) are separate.
- **MINOR** — this repo's backlog moves to the forge (`.ai-dev/config.json` `collaboration.backlog` `file → forge`, Operator decision): the whole backlog lives as GitHub issues, not `.ai-dev/backlog.md`. The `_collaboration` self-doc is reconciled (the forge-issue backlog adapter is now ACTIVE; the two `team`-gated readers stay inert while `team:false`), and `PROTOCOL.md ## Project config` is corrected to match — the backlog adapter is **`backlog`-gated** (active on `backlog:forge`, independent of `team`), a one-home fix of a stale "inert on `team:false`" statement about the backlog adapter. The legacy `.ai-dev/backlog.md` → issues migration is a separate follow-up.

---

## [5.57.0] — 2026-07-07

- **MINOR** — read-allow for the agent's OWN out-of-root scratch (invariant 2; backlog 2026-07-01, Operator-escalated P0). The `[mechanical]` boundary deny false-blocked a role reading its own harness-assigned scratch: the **tool-result overflow store** (a fetched doc/image whose body exceeded the inline cap) and the **per-session temp root** (the scratchpad, staged pasted files, task outputs). The live symptom: an agent could not recognise a fetched image — its spilled bytes lived under the overflow store and the deny treated that as "out of root." A read-only widening now admits two sanctioned roots, derived **fail-closed** from the Claude env (mirrors `componentRoots`): each path is admitted only if its env is present, `fs.realpathSync` resolves it (the dir exists), and it passes the overbroad guard (not a filesystem root, not an ancestor of / equal to the session root, not the shared `<slug>` parent that would leak a sibling session — only the session-id child). Bad/absent env ⇒ empty set ⇒ byte-identical to today; the two paths derive independently. **Read-only by design:** the read-family predicates (`pathOutsideRoot`/`findTargetOutsideRoot`/`bashReadTargetOutsideRoot`) consult the new `input.sanctionedScratch` alongside `isInsideAnyComponent` (via `isInsideSanctioned` in `engine-paths.mjs`); the **write** predicate does NOT, so a write outside the root stays denied even to a sanctioned path — the widening leaks nothing out, only lets the agent see what the platform already wrote for it. **Session-id-narrow:** the temp entry is scoped to THIS session's id, so a sibling session's transcripts stay denied. Core/adapter split preserved: the engine stays neutral (a passed allow-set, no env knowledge); `src/adapter/claude/sanctioned-scratch.mjs` owns CC's slug + temp conventions and threads the set onto the neutral input; the shim threads `input.home` so a bash `~/…` reference can reach a sanctioned path. opencode parity (its deny is a plugin) is the documented follow-up — the `input.sanctionedScratch` channel is the contract; v1 lands Claude (the Operator's platform). `PROTOCOL.md` invariant 2 + `## Enforcement` name the read-carve-out (read-only, fail-closed); `docs/architecture.md` `## Sanctioned scratch` is the one-home mechanism; `docs/decisions/out-of-root-scratch-allow.md` records the rationale, the read-only/session-narrow scoping, the CC-version residual (existence-check backstop), and the opencode follow-up. Tests: `src/adapter/claude/sanctioned-scratch.test.mjs` (derivation + fail-closed + guards + the read/write asymmetry + the sibling-session denial). Labelled honestly: a guarded, independently-reviewed widening of a `[mechanical]` floor, not "no risk" — it does not allow writes outside root, does not allow reads of sibling sessions, and does not allow reads of `CLAUDE_CONFIG_DIR` broadly. MINOR — a new adapter enforcement mechanism (a new capability in the deny layer), backward-compatible (a deny loosened, never tightened; downstream byte-identical with no/absent env); no contracted surface change (no MCP tool / HTTP route / event / schema / public API).

---

## [5.56.3] — 2026-07-06

- **PATCH** — a lazy `quality-toolkit` offer + a per-stack catalog (#332, part 1). A project that gained code after setup (or skipped setup step 5) had no reminder that `tools.json` was still the template, so real quality tools never got configured — the dogfood repo hit this with Rust (had to manually add `cargo fmt`/`cargo doc`/`cargo audit`). Two prose edits + one adjacent doc-drift fix, no core/code change. **(1)** `src/agents/procedures/setup.md` step 5 gains a **per-stack suggested-defaults catalog** (Rust / Python / Node-TS / Go — formatter, linter, type-checker, tests, secret scanner, UI/E2E), the one home for "the common set for stack X", framed as a starting point the project adjusts (reasoning still applies; existing tooling is honoured, not overwritten) — refining "never a hard-coded list" to "start from the catalogued defaults, adjust per project, never applied blind". **(2)** `src/agents/orchestrator.md` `## Side-tools` gains a **quality-toolkit** lazy offer (peer of doc-bootstrap/project-inception): fires when a stack marker is detectable (`Cargo.toml`/`pyproject.toml`/`package.json`/`go.mod`/…) AND `tools.json` is still the template (the `example-lint` row only); the body is setup step 5 (no separate procedure). **(3)** adjacent doc-drift: `src/quality/README.md` ("two `EXAMPLE-*` rows" → "one `example-lint` row" — the template carries exactly one). `[persona]` declinable offer — no deny layer, no mechanical floor; does not auto-populate `tools.json` without the Operator's word. PATCH — prose-only, no contracted surface change; part 2 (code-verbosity guideline) is a separate PATCH.

---

## [5.56.2] — 2026-07-06

- **PATCH** — the subagent safety-classifier signal made non-opaque + the routing boundary documented (#346, downstream-feedback). A spawned subagent can return the platform's opaque note that *"the safety classifier was unavailable when reviewing this sub-agent's work"* — Auto Mode's per-action check on the subagent couldn't run — with no prescribed fallback. Two gaps closed. **(1) Procedure (gap 1):** a fire-time-anchored bullet in `src/agents/orchestrator.md` `## Your seat` (where subagent results are accepted) — translate the note for the Operator in one plain line, run an explicit verification pass over the subagent's diff before acting on its output, and still ship the independent Reviewer: the fallback covers only the handback-to-gate gap, it does **not** replace the gate (invariant 3 is about our Reviewer; the platform classifier is orthogonal). `[persona]` discipline — the note is advisory, no deny can force the fallback. **(2) Routing boundary (gap 2):** a research pass in `docs/decisions/subagent-safety-classifier.md` — the classifier is a full second Claude LLM pinned to a concrete id (`claude-opus-4-6`/`claude-sonnet-4-6`), **not** a tier alias and **not** `guardModel` (the small-fast/background model), so `launch.aliases` does not route it (a concrete id bypasses tier-env — the same class as programmatic role-spawns); it rides `ANTHROPIC_BASE_URL`, so a foreign proxy covers it only by carrying a route for the literal id. The `.ai-dev/config.json` `_launch` docstring carries the SCOPE clause; the refuted hypothesis (classifier ≠ guardModel) and the version-dependent open residual are recorded. No `PROTOCOL.md` change (thin-core preserved), no deny-layer touch. PATCH — procedural/doc-only, no contracted surface change (no MCP tool / HTTP route / event / schema / public API).

---

## [5.56.1] — 2026-07-06

- **PATCH** — sub-agent workspace teardown via a periodic reclaim (#325). The spawn/continue contract has no teardown point and ship beat 5 deletes only the feature's plan/stamp, so a background sub-agent's worktree + scratch branch falls through every cleanup net and accumulates (the #325 incident: ~107 abandoned `worktree-agent-*` worktrees, ~117 branches, ~17 stray stashes). Closed with a periodic reclaim, not a heavy new adapter contract point: `PROTOCOL.md` gains a `gc` side-tool (an alias for the audit's workspace-hygiene dimension, runnable on demand); `src/agents/procedures/audit.md` (+ deployed `.ai-dev/procedures/audit.md`) gains a **workspace-hygiene** dimension — detect (`git worktree list` + `git branch`) and prune merged/abandoned `worktree-agent-*` worktrees + scratch branches; `src/agents/orchestrator.md` `## Audit` names the dimension; new `docs/decisions/scratch-regenerated-files.md` records the compounding mechanism — a tracked scratch-regenerated file (`.secrets.baseline`-class) manufactures false-dirty worktrees + stray stashes, so gitignore it, never track. `[persona]` procedure + a catalog alias — no new adapter contract point, no per-spawn cleanup (periodic/on-demand), no remediation of existing debris (preventative; a leaky downstream runs `gc` once). PATCH (no new code/agent/command/module — the `gc` side-tool is an alias over existing audit machinery).

---

## [5.56.0] — 2026-07-06

- **MINOR** — the resume pointer is the unconditional FIRST action, backed by a mechanical SessionStart inject hook (Operator ask 2026-07-06: «the model ALWAYS knows it has checkpoints + a pointer, and even in a strange context the first thing it does is check the status»). The crash-resume pointer-read was one prose line in the orchestrator — lost on a resumed/compacted session where the loaded context is already stale. Now: `src/adapter/claude/shim.mjs` short-circuits `SessionStart` to a fixed inject — the FIRST-action reminder (read `.ai-dev/state/current.md` before acting), firing on every source (startup / resume / clear / compact — the recovery cases), no engine evaluation (SessionStart carries no act). `src/adapter/claude/hooks.json` gains a SessionStart group (the third wired event; the installer's `mergeHooks` wires it for downstream); `.claude/settings.json` carries it. `src/agents/orchestrator.md` sharpens the resume line to unconditional (the first action of any turn that resumes OR when context feels degraded/contradictory/unfamiliar) and names the SessionStart inject as the mechanical backstop. New ratchet in `install-claude-wiring.test.mjs` (4 sources emit the inject; a UserPromptSubmit payload is not hijacked; RED-without-fix verified). The inject POINTS at the pointer path (it does not restate the status content — invariant 6); the orchestrator line is the one durable-text home (no second home in CLAUDE.md). Claude-adapter mechanical; opencode has no SessionStart realization → persona fallback (the sharpened orchestrator line). MINOR because a new wired hook event is an additive enforcement capability downstream projects gain on install.

---

## [5.55.2] — 2026-07-06

- **PATCH** — contracts are consulted during build/review, not just created and ship-re-checked (#349). The `docs/contracts/` home existed and was re-checked at ship for user-facing changes, but no beat forced a role to READ the touched contracts during development — so a contract was routinely violated and passed review (the #349 downstream report). Closed with a consultation discipline across the three role checklists: `src/agents/planner.md` gains a "Contracts touched" item (the plan enumerates the touched `docs/contracts/*` entries — an implicit set drifts); `src/agents/builder.md` gains a Contracts step (read each touched contract, the change satisfies each, a violation is a plan-level fork escalated — never decided silently — and a changed contract lands its entry + test at build); `src/agents/reviewer.md` strengthens "Contracts regression" → "Contracts" (proactive: take the touched list from the plan, read each, check the diff against it; a VIOLATION blocks, not only a missing entry; the three substrate axioms — naming honesty / derive-never-stamp / no-event-in-the-void — are the standing contract-review lenses, sharpening the existing naming / one-home / correctness items, not duplicating them). Procedural `[persona]` discipline; no `[mechanical]` floor or contract SURFACE changed; PROTOCOL.md untouched (core/adapter split — "the review checklist" is the mandate surface). Assembled agents regenerated to match.
...
- **MINOR** — *(shipped then reverted in 5.54.0 — kept for history)* Auto-compact checkpoint-restart for proxied non-Claude Claude Code sessions. A new Claude-adapter hook `compact-monitor.mjs` estimates context-window usage from the transcript JSONL tail on every prompt and, at a configurable threshold (default 0.5), injects a structured checkpoint instruction — reconcile `.ai-dev/state/current.md` + write a notes file (goal / progress / open items / next step) — and asks the Operator to restart with `claude --continue`. A companion `PreCompact` hook blocks the harness's model-summarize entirely on proxy sessions (it breaks on non-Claude models routed through `ANTHROPIC_BASE_URL`); on native-Claude sessions both behaviours are no-ops (auto-detected via `ANTHROPIC_BASE_URL`). Runtime threshold and the on/off toggle live in the gitignored `.ai-dev/state/auto-compact.json` (conversational adjust, no commit); the escape hatch is `enabled:false`. Zero new dependencies (Node builtins only); ships through the installer so downstream proxy projects get it automatically. The enforcement `shim.mjs` is untouched — the monitor is a standalone script with no engine rule logic. Design grounded in `docs/decisions/auto-compact-non-claude.md`; four live-verify items (JSONL `usage` field path, token-count semantics, `PreCompact(auto)` timing, `checkpointTriggered` reset across `--continue`) remain open for a real modelpipe/GLM session and are recorded there.
