...
- `meta/audits/2026-05-24_first-prod-run-feedback.md` — NEW: sanitized feedback с первого live test'а template'а на реальном downstream проекте (~3 дня работы, ~5500 LOC foundation, 1 feature spec). **Empirically proves:** token cost real, AP-1 fragile, conditional skip + lazy loading нужны
...
|---|---|---|
| P0 (срочно) | Silent-break gaps 1-3 + **AP-24 ADR auto-extraction** (от live test) + post-refactor defects (B-1..B-4) + size gate quick win | **v0.2.0** |
...
| P1 | Path C careful consolidation (same-axis only, ~25→~18) + **lazy foundational loading** (от live test) | **v0.3.0** |
...
#### Scope C: AP-1 enforcement (new P0 от live test)

9. **AP-24 — ADR auto-extraction enforcement:**
   - Новый spec-linter check в `check-spec-discipline`: detect sections с keywords `инвариант`, `архитектура`, `trade-off`, `decision`, длиной > 30 LOC → reviewer suggestion «extract в ADR»
   - Hard-block если spec section > 50 LOC architectural content без ADR reference
   - Новый AP-24 в `anti-patterns.md`: **«Architectural decisions buried in spec без ADR»**

   **Relationship с AP-1** (reactive ADR при architectural fork):
   - **AP-1** — *proactive* enforcement в `planner.md` Step 2: planner детектит fork (выбор между альтернативами), AI обязан создать ADR с reasoning, ссылка кладётся в plan
   - **AP-24** — *retroactive* enforcement в reviewer Step 7: spec section с архитектурным контентом > 30 LOC → suggestion / > 50 LOC без ADR ref → block
   - **Boundary heuristic:**
     - AP-1 для **architectural forks** (выбор между альтернативами, влияет на дальнейшие decisions)
     - AP-24 для **decision documentation** (фиксация constraints, invariants, trade-off'ов, которые применили)
   - **Precedence:** если AP-1 fired и ADR создан с reference из spec'а → AP-24 принимает spec section как OK даже > 50 LOC (поскольку summary в spec'е + детали в ADR — legit pattern)
   - **Test fixtures (mandatory в реализации AP-24):**
     - (a) Fork detected → AP-1 fired → ADR created → AP-24 quiet (spec references ADR)
     - (b) Fork missed AP-1 → spec has buried 50+ LOC decision → AP-24 blocks с suggestion «extract в ADR»
     - (c) Pure documentation > 30 LOC (constraints, invariants) → AP-24 suggests extraction, не blocks
   - Это enforcement существующего AP-1 — closes proven gap на live test'е (AP-1 dead letter, decisions buried в spec без ADR)

#### Scope D: Operational quick win

10. **Size gate на reviewer fan-out:**
...
**Estimate:** 5-7 PR (review revision после plan review concerning-1):
- PR 1: B-1 mode renaming sweep (mechanical, ~21 файлов)
- PR 2: B-2 + B-3 (feature-spec frontmatter + feature-review verdict line + roster fix)
- PR 3: B-4 + High findings batch (scripts location + 5 High)
- PR 4: Gap 1 — spec→test mapping CI gate
- PR 5: Gap 2 — test fudging prevention + AP-23 (см. open question #1 ниже — semgrep vs git-diff)
- PR 6: Gap 3 — regression coverage + AP-14 extension
- PR 7: AP-24 ADR auto-extraction + size gate

Reality check: каждый gap отдельный PR — atomic per AP-19. Если AP-23 / AP-24 текстовые секции в anti-patterns.md небольшие — могут share PR с соответствующим linter check'ом. Размер каждого PR — small/medium.
...
**Тип gate'а: HARD** (per plan-review concerning-4) — `meta/experiments/<date>_silent-break-prevention.md` должен существовать в `main` перед merge первого v0.3.0 PR'а. Это обеспечивает что v0.3.0 архитектурный refactor не строится на unvalidated gap closure.
...
#### Lazy foundational loading (новый scope, от live test)

В Stage F subagent prompt'ах (planner / coder / reviewer):
- Parse spec frontmatter impact flags
- Read только корреспондирующие foundational docs (если `threat_impact: yes` → threat-model; если `persona_impact: yes` → personas; etc.)
- Read базовый minimum всегда (vision, scope) — без них нет grounding
- **Estimated savings:** 40-80% load reduction per session, 2-3× context savings на типичной фиче

Это **прямой fix RESUME note pattern** observed на live test'е.
...
- Migration script test'ован на synthetic копии downstream prod-run target state без потерь content (verification report green)
...
- [ ] downstream prod-run target `template-sync` PR проходит без потери content

---

## After v0.5.0 — first downstream prod-run

После v0.5.0 template считается готовым к real prod-run'у. First downstream prod-run target получает `template-sync` PR:
...
- Re-run discipline-advisor на downstream codebase — sanity check preset recommendation
- Operator выбирает preset based on advisory + own judgment

**Downstream prod-run target = first prod-run рефинированной системы.** Task #16 был closed до текущей mode matrix; реальный prod-run будет здесь.
...
1a. **Gap 2 detection mechanism — semgrep fine-grained или git-diff coarse?** (plan-review concerning-3) Semgrep — precise (детектит `toBe(X)` → `toBeGreaterThan(Y<X)`), но fragile к variantам assertion API. Git-diff coarse — robust, multi-stack, но упускает test-only weakening. **Рекомендация:** start with git-diff coarse (robust baseline, covers most cases — test files modified без production code change → require ADR ref), добавить semgrep specifically для известных weakening patterns как enhancement позже.
...
6. **first downstream prod-run timing — после v0.4.0 или ждём v0.5.0?** **Рекомендация:** v0.4.0 (architecture validated там, v0.5.0 — только copy).
...
| **First downstream prod-run** | — | First prod-run рефинированной системы через template-sync | 1 PR (downstream) |
