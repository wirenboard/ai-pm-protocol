...
- **No automated tests in the protocol repo by design — validation by use.** The template ships as documentation + config; it has no runtime to assert against. Validation is whether the next feature on the template (or on a downstream project) flows through the workflow correctly. **Source:** `doc/features/template-v2_plan.md` § "Test plan" — "Validation by use: this template's development follows the new workflow itself. No automated tests in protocol repo."
...
| `.github/workflows/auto-tag.yml` | The single CI workflow — release on push to `main`. |
