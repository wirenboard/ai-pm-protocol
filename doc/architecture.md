...
`docs/features/_index.md` (component → feature) was a write-only output, useful to no reader. It is replaced by `docs/product-map.md` (component → contract → the features + reviews that built it, plus an `Infrastructure` bucket for contract-less work) — contracts are the source of truth for behaviour, so the PM-facing view is inverted to lead with them. This map is **generated** from contracts/plans/reviews/git, never hand-filled, rebuilt wholesale by `pm-auditor` from source (Product map generation procedure). Each per-contract block is **value-first**: it leads with `Что даёт` (projected from the contract's `## User value`) + `Границы` (from `## Out of scope`), with the build-history table demoted below under a `Чем построено:` label — a render-only projection of fields that already exist (no new contract field or generator input), pure markdown with no HTML, and old-format maps (literal `Guarantees:` label) are detected and offered a regenerate at `/pm-plan` and `/pm-audit`. Alongside it, an **authored** `docs/product.md` front door now exists — owned by `pm-architect`, hand-written (not generated), funnelling the PM from Зачем → Что умеет сегодня → Документы → Функции. The per-feature "did all artefacts appear" checklist the index implicitly served moves to an explicit `pm-plan-checker` DoD item, with `pm-auditor` dimension 1 as the project-wide backstop. **This template repo is a deliberate exception** — it has no user-facing Product Contracts (its "product" is the protocol itself), so neither the generated `docs/product-map.md` nor an authored `docs/product.md` is produced here; the protocol's capabilities are documented in this `architecture.md` and in `doc/features/` instead. **Source:** `doc/features/contract-centric-product-map_plan.md`; `doc/features/product-md-front-door_plan.md`; `doc/features/product-map-value-first_plan.md`; self-audit 2026-06-02.
