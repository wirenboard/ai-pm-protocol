...
- `.claude/agents/pm-auditor.md` / the reviewer check — extend the existing "validator present in Pipeline + actually run" discipline to **"the AI-minimums are encoded in the linter config"** (a project with the minimums stated but un-enforced → note). (Exact owner — `pm-auditor` dim vs `pm-plan-checker` DoD vs `code-review` — settled in arch-review.)
