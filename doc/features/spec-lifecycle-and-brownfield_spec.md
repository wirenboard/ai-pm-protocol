...
**AP-30 (новый): Spec staleness** — feature spec не обновлён после implementation changes. Detection: git log на code files vs `last_modified:` в spec'е. Если code touched после spec last_modified > 30 days — warning. Hard fail если behaviour-changing.
...
- `bootstrap-legacy.md` agent — добавить «Full retrofit» branch к существующему 3-choice menu.
- Spec template `<topic>_spec.md.tmpl` — добавить optional `## Behaviour observed` + `## Invariants extracted` (для brownfield).
- AP-30 в `doc/anti-patterns/AP-30.md` (staleness).
- AP-31 в `doc/anti-patterns/AP-31.md` (cross-feature contradiction — Layer 3 territory).
...
- Layer 3 linter — false-positive rate <= 10% (target измеряется на HeartVault re-do A/B).
...
- Invariant extraction heuristics — regex baseline или LLM-call? Trade-off accuracy vs cost.
- AP-30 staleness threshold (30 days?) — empirical или config-настраиваемый?
...
- [ ] AP-30 + AP-31 файлы под `doc/anti-patterns/`.
...
- [ ] HeartVault re-do A/B test готов запуститься (separately tracked, не в этом PR).
