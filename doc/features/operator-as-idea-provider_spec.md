...
## Новый AP-32: Jargon-first operator communication
...
- Новый раздел в `development-protocol.md` — «Operator interface model» (Three-level + 6 triggers + 6 rules).
- **Расширение** уже существующей `## Operator-facing tone` секции (добавлена в PR #72) во всех **5 logical agents** — добавляем 6 rules + 6 escalation triggers, не дублируем уже-внесённый baseline. *(Файлов на диске 9: project-bootstrap router + 4 bootstrap-mode subagents после fragmentation в PR #69, плюс planner / coder / reviewer / release-helper. Bootstrap split — один agent identity для cache, не пять отдельных. Concept count = 5.)*
- Добавить AP-32 в `doc/anti-patterns/` (после prompt-economy C granularization файлов).
...
- AP-32 violations — **soft warn** в reviewer-agent (не CI block, не request-changes по умолчанию). Hard-fail задушит operator-facing communication.
...
**Сценарий 3: anti-pattern — jargon-first escalation (AP-32 catch)**
...
3. Reviewer ловит AP-32 в _review.md → request-changes.
...
- [ ] Все 5 logical agents (planner / coder / reviewer / release-helper / project-bootstrap) содержат «Operator-facing tone» секцию с 6 rules + 6 escalation triggers — для project-bootstrap rule применён в router + 4 bootstrap-mode subagents (9 файлов суммарно).
- [ ] AP-32 в `doc/anti-patterns/AP-32.md` с failure mode + examples.
...
- [ ] Examples-файл `_claude/operator-facing-examples.md` с 5 pairs (terse/verbose per logical agent: planner / coder / reviewer / release-helper / project-bootstrap).
- [ ] Regression test fixture с примером AP-32 violation, проверяемый через `check-spec-discipline.sh --regression`.
