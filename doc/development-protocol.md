...
Полное описание routine — `.claude/agents/bootstrap-template-sync.md` (specialized subagent, routed router'ом `project-bootstrap`).
...
### 6.1. Coverage blind spot + mutation testing

**Почему `per-diff coverage ≥ 80%` недостаточно:**

Coverage измеряет выполнение строк и веток. Добавление поля в объект (`return { ...existing, order: x, title: y }`) — это та же строка, та же ветка. Coverage не падает, тест проходит, поле не проверяется — **vacuous coverage**.

**Mutation testing** — следующий уровень. Инструмент автоматически меняет код (`order: x` → `order: undefined`) и запускает тесты. Если тесты не упали — «мутация выжила» → assertion на это поле отсутствует.

Инструменты: Stryker (JS/TS), mutmut (Python), cargo-mutants (Rust), pitest (Java).

**Стоимость:** mutation testing ~10× дольше обычных тестов — не включается везде по умолчанию.

**Прагматичный подход:**

| Сценарий | Механика |
|---|---|
| Обычные фичи | Gap 4 coder discipline (grep assertions для новых полей) |
| Security-critical код (auth / crypto / billing / PII) | Mutation testing обязателен — добавляется в CI на Stage D с `--only-covered-code` |
| Shared modules с высоким риском регрессий | Mutation testing рекомендуется |

На Stage C: если проект имеет security path или критичные shared modules — mutation testing включается в CI pipeline как отдельный gate (только для затронутых модулей, не full run).

---

## 7. AI-specific code linting

Стандартные linter defaults писались под human developers. AI имеет свой «акцент» — catalogue из 18 категорий. На Stage C каждая обязательно замаппена на конкретное правило стека, на Stage D конфиги генерируются.
...
| Невалидный синтаксис config-файлов | Добавляет trailing comma, неэкранированные символы в JSON/YAML/TOML | `JSON.parse` / `jsonlint` / `yamllint` / `taplo` на все `*.json`, `*.yaml`, `*.toml` в CI. Особенно критично для confed schemas, package.json, CI workflows — синтаксическая ошибка там = молчаливый fail |
...
| `adr-decision-component-bounded` (v0.7.0+, AP-27, family `cross-doc-bounded`) | Для каждого ADR в `architecture-decisions/` (кроме `0000-*`): если присутствует секция `## Decision` с subsection `### Components` (или `## Components`) — каждый named component должен иметь spec/plan/foundational reference (Markdown link или явная цитата `(see <file>:<anchor>)`). Если subsection отсутствует но Decision body содержит ≥ 2 backtick-named identifier'ов — warn (suggest Components subsection). Override: `[source-bounded-override: <reason>]` | Hallucinated decision component (concrete building block без trace к source) — invisible drift (см. AP-27) |
| `inter-adr-contradiction` (v0.7.0+, AP-28, family `cross-doc-bounded`) | Pairwise check ADRs изменённых в текущем PR (`git diff main...HEAD --name-only -- '*architecture-decisions/*.md'`). Heuristic: extract «red line» phrases из Alternatives секции ADR-A (`rejected`, `violates`, `prohibited`, `отвергнуто`, `нарушает`, `запрещено`), grep названия в Decision секции ADR-B. Conservative threshold (false positive rate priority) | ADR-A explicitly rejects pattern P, ADR-B Decision implements P под другим именем — invisible inter-ADR contradiction (см. AP-28) |
| `adr-feature-scope` (v0.7.0+, AP-29, family `cross-doc-bounded`) | Каждый ADR имеет frontmatter `feature_topic:` (binding к feature scope). Если поле отсутствует — fail. Если присутствует — cross-check Components vs spec этого feature_topic: components вводящие functionality описанную ТОЛЬКО в spec'е другого topic'а (по grep substring match) → flag scope creep. Override: тот же `[source-bounded-override: <reason>]` | ADR scope shifts beyond feature scope (multi-feature scope creep) — AP-1 violation (см. AP-29) |
| `spec-staleness` (v0.8.0+, AP-31, family `cross-feature-bounded`) | Для каждого merged `<topic>_spec.md` сравнивает `last_modified:` / `spec_approved:` с `git log` timestamps на implied code paths (heuristic: topic name + `affects_features:`). Если code touched > `staleness_days:` после spec date (default 30, configurable в `.bootstrap-state.md`) — warn. Skip: `lite-mode: bugfix`, `deprecated:` spec'и. Fixture testing через `STALENESS_FIXTURE_OVERRIDE` env. Override: `[spec-staleness-override: <reason>]` | Spec frozen в state «merged», но code modified > threshold days без spec update — silent ground truth drift (см. AP-31, § 9.6) |
| `cross-feature-invariant` (v0.8.0+, AP-33, family `cross-feature-bounded`) | Extract invariant statements из specs + foundational docs (vision/positioning/mvp-scope/threat-model) через regex baseline (`MUST NOT` / `ни при каких` / `запрещено` в `## Invariants extracted` section ИЛИ list-item form). Pairwise cross-check; scope optimization через `affects_features:` (O(N) targeted vs O(N²) pairwise). Hard fail при contradiction. Override: `[cross-feature-override: <reason>]` | F-N spec нарушает invariant F-M spec'а или foundational doc'а — Layer 3 cross-feature drift (см. AP-33, § 9.6) |
...
**Source contract** для каждого AI-агента — explicit list ground truth artifact'ов и определение fork'а — фиксирован в `.claude/agents/<agent>.md` секциях `## Source contract` / `## Fork-justification protocol` / `## Spawn discipline`. С v0.7.0 (agent-consolidation) — 5 agent файлов (project-bootstrap / planner / coder / reviewer / release-helper) + `CLAUDE.md.tmpl § Source-bounded discipline для orchestrator'а` для main session AI — итого 6 точек drift'а закрыты универсальным contract'ом. Раньше было 11 файлов + main session (12 точек); консолидация inlined 5 specialized reviewer ролей в `reviewer.md` как sections. С v0.7.x (prompt-economy Option B / PR-5) — `project-bootstrap.md` split на router + 4 mode-specific subagents (`bootstrap-greenfield` / `bootstrap-legacy` / `bootstrap-resume` / `bootstrap-template-sync`), каждый со своим source-bounded contract (per-agent specifics) — итого 9 agent файлов + main session, contract semantics те же.
...
**Cross-refs:** AP-25 (AI artifact extends beyond source — generic discipline), AP-26 (Orchestrator architectural injection — upstream defence через spawn-discipline), AP-1 (timing companion), AP-24 (LOC threshold companion), AP-6 (silent deviation generic principle), AP-27/AP-28/AP-29/AP-30 (Layer 2 cross-doc bounded extensions для invisible drift), AP-31/AP-33 (Layer 3 cross-feature bounded — § 9.6).

**Layer architecture (Layered defence):**

| Layer | Mechanism | Catches | Status |
|---|---|---|---|
| **Layer 0** | Spec frontmatter + `spec_approved` operator-touch | Bad-faith deviation | existing |
| **Layer 1** | Per-agent source contract + fork-justification + frontmatter линтер (`adr-spec-reference` / `plan-spec-reference`) | **Visible** drift в одном artifact'е (alternatives, frontmatter compliance) | v0.6.0+ |
| **Layer 2** | Step 2.5 auto-spawn reviewer + cross-doc check'и (`adr-decision-component-bounded` / `inter-adr-contradiction` / `adr-feature-scope`) + foundational auto-load в planner & reviewer | **Invisible** drift в одной фиче (decision components, contradictions, scope creep, plausibility bias) | v0.7.0+ |
| **Layer 3** | Cross-feature reviewer pass + cross-feature-bounded linter family (`spec-staleness` / `cross-feature-invariant`) | **Cross-feature** drift (F-N нарушает invariant F-M, spec staleness, foundational contradiction) | v0.8.0+ (§ 9.6, AP-31/AP-33) |
| **Layer ∞** | Anchored regression test fixtures across template versions (`doc/_templates/regression-cases/`) | Long-term protocol erosion / template regression | v0.7.0+ (fixture infrastructure) |

Pattern continues: каждый layer ловит class drift'а который previous слой не ловит.

### 9.6. Cross-feature invariants (Layer 3)

Layer 3 anti-drift закрывает class который Layer 1 (source-bounded внутри artifact'а) + Layer 2 (cross-doc-bounded внутри фичи) пропускают: **cross-feature contradiction** — F-N spec нарушает invariant установленный F-M spec'ом или foundational doc'ом.

**Failure mode** (anchor pattern из spec):

F-01 устанавливает invariant «server не имеет plaintext access». Через 3 месяца F-07 (export) добавляет server-side temp storage. F-07 spec не упоминает F-01 (independent feature framing). PM не помнит invariants F-01 после 10 features. Silent break без cross-feature pass'а.

**Two anti-patterns:**

- **AP-31 — Spec staleness** (severity `warning`). Code drift от spec'а > threshold days без spec update. Detection: `git log` на implied code paths vs spec `last_modified:`. Threshold configurable через `staleness_days:` в `.ai-pm/.bootstrap-state.md` (default 30).
- **AP-33 — Cross-feature invariant contradiction** (severity `critical`). F-N invariant противоречит F-M / foundational invariant. Detection: regex extract «invariant statements» (`MUST NOT` / `ни при каких` / `запрещено`) из specs + foundational docs, pairwise cross-check.

**Linter family `cross-feature-bounded`** (см. § 9.1 catalogue, scripts/check-spec-discipline.sh):

- `spec-staleness` (AP-31) — warn-only по умолчанию.
- `cross-feature-invariant` (AP-33) — hard fail по умолчанию.
- Override markers: `[spec-staleness-override: <reason>]` для AP-31, `[cross-feature-override: <reason>]` для AP-33 — pattern identical AP-25 `[source-bounded-override:]`.
- Wrapper script `scripts/check-cross-feature-invariants.sh` (тонкий, инвоучит family через `--check`) — для CI invocation и acceptance criterion.

**Reviewer Step 2.5 Layer 3 pass** — обязателен **при одном из triggers**:

- New F-N spec creation (always — linter runs cheap).
- `topology_impact: yes` в frontmatter F-N.
- Foundational doc change в том же PR.
- `affects_features:` populated в frontmatter F-N (explicit cross-feature scope).
- Invariant overlap >= 2 between F-N spec и foundational / другие spec'и (linter flag).

**Hard cross-feature triggers** (mandatory operator-gate — fork-justification protocol, см. § 9.5):

1. F-N меняет foundational doc (vision / positioning / mvp-scope / threat-model).
2. F-N имеет shared component с другим F-M (через `affects_features:` frontmatter list).
3. Invariant overlap >= 2 between F-N и foundational + другие spec'и.

При hard trigger — reviewer escalates в business terms (см. AP-33 § «Как поступать вместо» → fork-justification на cross-feature contradiction).

**Optional spec template sections** (см. `_templates/feature-spec.md.tmpl`):

- `## Behaviour observed` — descriptive what-code-does (для brownfield retrofit'нутых spec'ов).
- `## Invariants extracted` — explicit invariant list (precision-scope для AP-33 regex baseline).

**Optional frontmatter fields:**

- `affects_features: [topic1, topic2]` — cross-feature scope hint. Populated при `topology_impact: yes` или явном shared component. Linter scope optimization (O(N) targeted vs O(N²) pairwise).
- `last_modified: YYYY-MM-DD` — для AP-31 staleness compare.
- `deprecated: YYYY-MM-DD` — opt-out of staleness checks.

См. `doc/anti-patterns/AP-31.md` + `doc/anti-patterns/AP-33.md` + `doc/features/spec-lifecycle-and-brownfield_spec.md`.

### 9.7. Brownfield adoption (В1 / В2 flow)

Шаблон originally designed для greenfield (Stage A-D = bootstrap). Brownfield = existing repo с features БЕЗ spec'ов. **5 сценариев матрицы:**

| Сценарий | Стартовое состояние | Routine |
|---|---|---|
| **А** Новый продукт | Pustoy repo | `bootstrap-greenfield` Stage A-D → F-01 silent build |
| **Б1** Активный + новая фича | Все existing F-N имеют spec'и | `<topic>_spec.md` draft → Layer 3 cross-check existing spec'ов → silent build |
| **Б2** Активный + bug fix | Все existing F-N имеют spec'и | failing test first → inline edit `last_modified:` в affected spec'е → fix |
| **В1** Brownfield + новая фича | Existing F-1..F-N БЕЗ spec'ов | **Hard floor:** extract spec'и всех features перед F-N+1 (Choice 4 retrofit), либо **soft:** legacy-quick adoption + per-feature mini-research |
| **В2** Brownfield + bug fix | Existing F-N БЕЗ spec'ов | Targeted extract spec ТОЛЬКО affected feature → bug-fix flow Б2 |

**В1 Full retrofit flow** (4 steps — реализовано в `.claude/agents/bootstrap-legacy.md § Choice 4`):

1. **Audit:** scan `**/*.{ts,py,sql,...}` → group by feature heuristic-based (directory structure, route boundaries, module ownership). Output: predicted feature list.
2. **Operator review:** confirm / rename / split / merge features.
3. **Spec extraction:** per approved feature — generate `<topic>_spec.md` skeleton:
   - Frontmatter (impacts inferred), `extracted: yes` marker.
   - `## Behaviour observed` — descriptive what-code-does.
   - `## Invariants extracted` — assertions / type constraints / error handling. **Better miss than wrong** (0% FP target).
   - `## Open questions` — explicit list что AI не смог determine.
4. **Operator approval:** PM fills `Open questions` per spec, помечает `spec_approved:`. После — F-N+1 идёт стандартным flow Б1.

**В2 Targeted extract flow** — для bug-fix urgency:

1. **Detect bug area:** failing test или operator description → AI mapping на existing files.
2. **Targeted extract:** generate spec **только** для affected feature (один `<topic>_spec.md`, ~100 LOC). Не extract'ить всю topology.
3. **Bug-fix flow:** failing test first → fix → spec gets `last_modified:` + `## Bug fixes` log entry.

**Trade-off (acknowledged):** В2 leaves остальной repo без spec'ов. Layer 3 не работает полноценно для не-extracted features. Acceptable per urgency; gap closes incrementally при touch'е других features.

**AP-31 staleness applies ко ВСЕМ spec'ам** включая retrofit'нутые (per operator decision plan.md Q2). Retrofit'нутые spec'и могут начинать жизнь stale (extracted from old code) — surface как warning, operator решает per-case (update spec / mark deprecated / acknowledge через `[spec-staleness-override:]`).

**Hard floor для retrofit (Choice 4):**

- Retrofit extraction = **descriptive only**, не business intent inference. Operator approval mandatory per spec.
- `spec_approved:` ставится только operator'ом, не AI.
- `## Open questions` non-empty в extract'нутом spec'е — empty == suspicious shallow extraction, escalate.

См. `doc/features/spec-lifecycle-and-brownfield_spec.md` + `.claude/agents/bootstrap-legacy.md § Choice 4` + AP-31/AP-33.
...
| 2.5. Cross-doc review (Layer 2) | AI reviewer subagent | cross-doc verdict в чате + `_review.md` draft trail | Verdict embed'ится в plan summary до operator-approval'а (см. ниже + AP-27/AP-28/AP-29/AP-30) |
...
**Step 2.5 (cross-doc review, Layer 2) — обязателен для plans с новыми ADRs OR foundational area impact.** Layer 2 anti-drift ловит **invisible drift** который Layer 1 (per-agent source contract + frontmatter линтер) пропускает: hallucinated decision components (vs hallucinated alternatives), foundational doc drift, inter-ADR contradictions в одной PR, multi-feature scope creep, plausibility/structural bias. Mechanism — mandatory auto-spawn reviewer-agent с расширенным source contract (cross-doc foundational scan + inter-ADR pairwise check + scope creep detection). Verdict embed'ится в plan output summary до того как оператор marks `plan_approved:`.

**Triggers Step 2.5:**

- В plan'е новые ADRs (один и более) — обязательно, без exception
- Spec frontmatter `topology_impact: yes` OR `threat_impact: yes` OR `scope_impact: yes` — foundational area touched
- Plan touches > 1 architectural area (judgement call planner'а, если flagged)

**Cannot skip:** skip-marker требует explicit operator-justification в HEAD commit body `[step-2.5-skip: <reason>]`. Pattern identical AP-23 `[test-modify-override:]` / AP-16 `[review-override:]` — оператор знаком с механикой. Skip применим для **только** trivial / lite-mode plans без новых ADRs где Layer 1 покрытие достаточно.

**Output:** cross-doc verdict (approve / approve-with-comments / request-changes) + список findings типа `AP-27` (hallucinated decision component) / `AP-28` (inter-ADR contradiction) / `AP-29` (ADR scope creep) / `AP-30` (plausibility bias). См. reviewer.md § «Step 2.5: Cross-doc review (Layer 2)».

**Rationale:** Layer 1 (текущий) ловит visible drift (alternatives с invented options, missing frontmatter). Layer 2 caught на live test'е 2026-05-25 второй класс drift'а в already-Layer-1-approved ADRs. Без mandatory enforcement — это soft suggestion которое теряет в реальности (тот же failure mode как «reviewer-agent должен запускаться» — observed pattern в anchor incident). См. AP-27 / AP-28 / AP-29 / AP-30 в `anti-patterns.md`.
...
- **reviewer** — независимый review через inline sections (Mandatory baseline + one Domain). Mandatory all modes. Architectural-context + learning-oriented findings. v0.7.0+: 5 ранее specialized reviewer файлов inlined в этот файл как sections.
- **release-helper** — changelog, version bump (SemVer), release PR. Не делает release сам.

**Retired в v0.7.0** (agent-consolidation): `protocol-compliance-reviewer.md` / `backend-reviewer.md` / `frontend-reviewer.md` / `design-reviewer.md` / `database-reviewer.md` — inlined в `reviewer.md` как sections (Bug #3 — nested spawn не работал reliably). `discipline-advisor.md` — retired (PoC accuracy gate ≥80% per axis never validated; hard floor → `scripts/check-security-floor.sh`; reprompt → `scripts/check-skip-reprompts.sh`).

**Split в v0.7.x** (prompt-economy Option B / PR-5): `project-bootstrap.md` 733 LOC → router (~180 LOC) + 4 mode-specific subagents (`bootstrap-greenfield` / `bootstrap-legacy` / `bootstrap-resume` / `bootstrap-template-sync`). Per-spawn token cost dramatically lower — каждая сессия грузит router + ровно один relevant subagent, а не monolith.
