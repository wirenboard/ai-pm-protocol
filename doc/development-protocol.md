...
Универсальный protocol разработки в связке **соло-оператор + AI-агент**. Оператор — это PM (Trust profile A) или developer (Trust profile B/C), который ведёт продукт один; convention из anti-patterns.md AP-16. Описывает **процесс**, не технологию: стек и инструменты выбираются на Stage D и генерируются на Stage E под конкретный проект.
...
│   ├── legal.md
│   ├── maintenance-playbook.md    ← optional (Stage D)
...
Если recipe для стека отсутствовал — драфтился совместно с оператором на Stage D. После успешного Stage E result commit'ится в `doc/_recipes/cache/<aspect>-<stack>.md` для следующих проектов.
...
| `adr-only-from-plan` | Каждый ADR в `architecture-decisions/` создаётся в commit'е, который также содержит `<topic>_plan.md` (или в Stage D bootstrap commit для foundational ADRs) | ADR появился вне правила § AP-1 |
...
Четвёртая категория линтеров (после Code / Architecture / Spec). **Особенно критично для оператора при Trust profile A**, который не читает AI-generated код: security-уязвимости должны ловиться tools'ом, а не human review.
...
- [ ] `tech-stack.md` (включает topology / stack / db references) (Stage D)
...
**Step 7 (reviewer) — обязателен для всех modes**, не только rework mode. Причина: оператор при Trust profile A не читает код, и без независимого review нет никакой human-level гарантии, что код = plan'у. Reviewer-agent работает в чистом контексте, не знает деталей implementation'а до review, выдаёт `<topic>_review.md` с severity-tagged findings. Оператор (Trust profile A) читает только review (формализованный markdown), не код. Trust profile B/C — может читать diff дополнительно, см. `coder.md` / `reviewer.md`.
...
Если оператор использует GitHub — это настраивается через `gh api` в Stage E. Если другой SCM — overlay указывает эквивалент.
...
**Детекция (init-agent на Stage E):** попробовать `gh api repos/<owner>/<repo>/branches/main/protection`; если ответ `403` с сообщением `Upgrade to GitHub Pro…` — мы в free + private. (Для других SCM — аналогичная проверка по их API; если серверная защита недоступна на текущем тарифе, логика та же.)
...
- Отменить Specification First, Tests First, operator-gate, ADR-реактивно, operator-never-reads-code (при Trust profile A).
