...
Универсальный protocol разработки в связке **соло-оператор + AI-агент**. Оператор — это PM (Trust profile A) или developer (Trust profile B/C), который ведёт продукт один; convention из anti-patterns.md AP-16. Описывает **процесс**, не технологию: стек и инструменты выбираются на Stage C и генерируются на Stage D под конкретный проект.
...
AI-агенты — исполнители принятых решений; любой output bounded by source artifacts agent'а, любое расширение beyond source требует operator-touch через fork-justification protocol (см. § 9.5 + AP-25 + AP-26). Без этого contract'а template'а нет смысла: PM (Trust profile A) не может trust AI-output, потому что в любой точке цепочки AI мог тихо расширить scope.
...
Принцип одной строкой из § 1.5.1 — здесь его detail. Закрывает gap, который остаётся после AP-1 / AP-24 / AP-6 / Trust profile A learning layer'а: **content** drift в AI-produced artifact'ах (не только timing, не только LOC threshold, не только substantive justification).
...
**Универсальный fork-justification protocol** (формат фиксирован — Trust profile A reliable yes/no без code review):
