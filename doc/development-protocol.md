...
| **Layer 1** | Per-agent source contract + fork-justification + frontmatter линтер (`adr-spec-reference` / `plan-spec-reference`) | **Visible** drift (alternatives, frontmatter compliance) | v0.6.0+ |
| **Layer 2** | Step 2.5 auto-spawn reviewer + cross-doc check'и (`adr-decision-component-bounded` / `inter-adr-contradiction` / `adr-feature-scope`) + foundational auto-load в planner & reviewer | **Invisible** drift (decision components, contradictions, scope creep, plausibility bias) | v0.7.0+ |
| **Layer 3** | Anchored regression test fixtures across template versions (`doc/_templates/regression-cases/`) | Long-term protocol erosion / template regression | v0.7.0+ (fixture infrastructure) |
