...
**Какой mode преимущественно использует.** `new-product` (первый продукт через template) → `feature` (фичи в собственных продуктах) → реже `rework`. Периодически — `template-sync` (bump версии template'а в существующих своих проектах).
...
**Shared pain (см. project framing):** «умею писать код, но не всегда уверен, что делаю **полезную** фичу, а не technically correct unnecessary». Template даёт **PM-discipline** (Stages A-C, persona/journey/scope linkage) как product-mindset coach, не только code-discipline.
...
**Какой mode преимущественно использует.** Зависит от проекта; чаще всего `feature` и `rework` в собственных или поддерживаемых проектах. Для quick fixes использует **`bug-fix`** mode (lite ceremony) — особенно характерно для Persona C, которая хочет lighter ceremony для small changes.
...
| Developer (B/C) | Template как PM-discipline (Stages A-C, enforced spec linkage) | «что делать полезного» |
...
- **Developer (B/C)** через PM-discipline (Stages A-C, persona/journey/threat-model thinking) постепенно осваивает **продуктовое мышление** — почему фича нужна именно так, как она привязана к persona, какие edge cases важны.
...
3. **Tagging «эту категорию я ревьюю сам» (Persona B)** — реализуется в `.ai-pm/doc/ai-linting-rules.md` через **tier-метки** per категория:
   ...
   Bootstrap-agent на Stage D генерирует defaults — все `tier: enforce`. PM редактирует ручно после первой new-product mode итерации.
