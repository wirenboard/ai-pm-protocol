...
**Failure mode common pattern**: AI agent видит «логичное расширение» source artifact'а — недостающая alternative, очевидная validation, helpful default — и **молча реализует** в своём output вместо surface'нуть оператору как fork. Operator при Trust profile A не имеет mechanism'а это поймать post-hoc.
...
2. **Универсальный fork-justification protocol** (формат фиксирован для всех agents — единый pattern для Trust profile A):
...
- *Legitimate routing:* orchestrator spawn'ит planner'а с промптом «Plan для фичи `agent-source-bounded` per approved spec. Repo: <path>. Branch: feature/<topic>. Trust profile: A. Mode: feature.» — pure routing, никаких архитектурных директив.
