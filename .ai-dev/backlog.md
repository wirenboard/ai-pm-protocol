...
- **Adapter-filter Option B — filter the Claude orchestrator (feature #3 completion).** 5.28.0 shipped the platform-filter MECHANISM + tag, but the Claude orchestrator loads RAW via `CLAUDE.md @import`, so the filter doesn't reach it — no Claude-side noise reduction yet. Option B: assemble a `platform:claude`-filtered Claude-orchestrator artifact, repoint the import, drift-guard it — a structural change to the load realisation + downstream wiring, **surfaced for the Operator** (PR #284), awaiting an explicit go. Until taken, the tag/mechanism only filters the OpenCode-assembled agents.
