...
**Diagnostic pending (run in the downstream repo to disambiguate install-bug vs design-hole):** confirm `.opencode/plugins/` carries the entry plugin AND that a known mechanical deny actually fires (e.g. a write into `.ai-dev/tooling/` must throw `[ai-dev] …`). A pass there = dead plugin (install/wiring bug, add to this entry); a throw = live-but-leaky (the F1–F5 design holes stand).
