...
## Audit dispatch — post-multi-user sweep (v5.26.0) — 2026-06-28

Whole-tree audit after the 8-feature multi-user batch. HEALTHY, 0 BLOCK/HIGH. The fix-now items (MED-1 orchestrator stale-honesty, LOW-3 design-doc config shape, NIT-1 `--body-file -` safety) shipped in the dispatch PR. Remaining, recorded here:

- **MED-2 — the dogfood repo doesn't eat its own size-linter recommendation.** `## Setup` step 5 recommends a per-file size/complexity rule (~800) and backlog F7 homes it, but `eslint.config.mjs` carries no `max-lines`/`complexity` rule — so `engine.mjs` (966), `install.test.mjs` (858), `install.mjs` (813) sit over the line invisibly; only a manual audit catches them. Fix direction: add an eslint `max-lines` (and/or `complexity`) rule tuned soft to the repo's own recommended threshold, so oversized files become a mechanical CI finding. The most actionable systemic item from this audit. Effort S.
- **LOW-1 — `engine.mjs` (966 lines) decomposition seam.** Cohesive but splits cleanly along: path/root resolution · bash-command parsing · git/merge-topic logic · config reading · the predicates + `evaluate`. A behaviour-preserving `decompose` (`## Decompose`, tests-first) candidate; the parity/merge-gate/components suites are the behaviour net. Siblings over the line: `install.test.mjs` (858), `install.mjs` (813). Effort M, multi-PR.
- **LOW-2 — `orchestrator.md` past its own extraction trigger (confirms N1).** Now 365 lines (trend 297→312→321→323→365); the multi-user feature added `## Backlog` + ship bullets. Seam: extract the rarely-fired side-tools (inception, doc-bootstrap, threat-discovery, 8D, elicitation, research, decompose, upgrade, downstream-feedback) into `src/agents/procedures/*.md` stubs (the `parallel-work.md` precedent), leaving the always-on loop/seat/setup prose in the floor body. Folds the existing N1 entry. Effort M.
- **LOW-4 — 3 moderate dev-only CVEs, accepted.** `npm audit`: js-yaml + markdown-it (transitive under `markdownlint-cli2`), a quadratic-DoS class over the repo's OWN trusted markdown — no untrusted-input path, and the fix needs a breaking `markdownlint-cli2` downgrade. Recorded as a conscious accept (re-check on the next `markdownlint-cli2` major). Effort 0 (watch).
...
**Thesis (aligns with our own "persona floor" intake + the H1 lesson + the code-graph prosthesis split):** the gates catch "doesn't crash", not "does what it claims". Semantic correctness slips through. **Lever — move checks persona→mechanical where possible; where not, make the Reviewer checklist CLASS-SPECIFIC, not a generic "check quality" a persona can answer "LGTM" without looking in the right place.** The mechanical ones are the only checks independent of persona conscientiousness.

Seven findings, mapped to protocol homes (priority/sequencing TBD):

- **F1 — Facade subsystems (code looks alive, is inert).** Pattern: a pipeline declared *N-level* with one level a hardcoded stub that gets continued past; a component named "learning/adaptive" that never persists state, so cannot learn; ceremony-heavy code wrapping a trivial condition. Loop-blind because Plan fixes *what to build* with no gate for *does it do what it claims*, and tests assert "doesn't throw", not "the subsystem performs its function". Homes: (a) **`builder.md` plan checklist** — any plan claim of the form *learns / adaptive / N-level / evolves* must carry either a test that **exercises the claimed behaviour** or an explicit `PLACEHOLDER` mark (incomplete-by-design, declared); (b) **`reviewer.md`** new item — "does the subsystem do what its name/docstring promises? is the described state persisted across restarts? any level/branch that is de-facto inert?". Likely a **code-kind capability module** (cheapest, highest value against decorative AI-code). Effort S. Dedup: sharper than the resolved new-path-coverage floor (5.16.2) — that asks "is there a test", this asks "a test of the *claimed behaviour*".

- **F2 — Silent failure suppression (mechanical).** Pattern: a periodic op whose call throws (e.g. a missing required arg) and the exception is swallowed by a blind `except` — the feature "exists" but is dead; same for suppressed migration/teardown failures; green tests because the throw is eaten. Direct descendant of the persona-floor intake's point 6. Home: **`## Setup` step-5 quality-toolkit proposal must recommend blind-except / try-except-pass rules as ERRORS** for the stack (Python: ruff `BLE001` + bandit `S110`/`S112`; equivalents elsewhere). Mechanical, no persona. **Note:** our core ships only the toolkit-proposal *guidance* — the downstream brings the tool; the protocol change is the recommendation, not a hardcoded linter. Effort S.

- **F3 — No failure is swallowed (Reviewer).** Companion to F2 for what a linter can't see: **`reviewer.md`** item — "every `except` either logs or re-raises; a 'defensive' try that converts broken code into silently-not-working code is the anti-pattern, blocks." Effort S.

- **F4 — Isolation / identity invariants treated as background, not contract (conditional).** Pattern: cross-user data leakage (a recall path with no owner filter; a new table missing the visibility column); an unauthenticated endpoint returning an identity-binding secret. Loop-blind because the threat-model + user model are read on Understand as *background* and never become Review-beat *checks* — a stamp exists, but it checked "works", not "holds the isolation invariant"; a SAST linter can't see semantics. Homes: (a) **`builder.md` plan checklist**, conditional trigger (per-user / visibility / identity surface) — the plan must record the isolation invariant as a contract string (who sees what; who may issue the identity-binding action); (b) **`reviewer.md` separate security-adjacent dimension**, same trigger — "every new SQL/recall path filters by owner? new table inherited the visibility column? every mutating/identity endpoint authenticated or explicitly marked accepted-risk in the threat-model?". Belongs in the **threat/security capability module** (trigger-gated), NOT the always-on core. Effort M.

- **F5 — A parallel path doesn't inherit the original's protections (Reviewer).** Pattern: a streaming path missing the timeout / cost-accounting / circuit-breaker its blocking twin has; fire-and-forget tasks whose handles are GC'd. Loop-blind because the second path is built later and reviewed *in isolation*; the manifest's "no duplication / one home" is framed for **text**, not for **semantically-paired code paths**. Home: **`reviewer.md`** item — "if a feature adds a path parallel to an existing one (stream↔sync, batch↔single), enumerate the original's guards (try/circuit-breaker/cost/limits) and check each is present in the new path." Consider extending the manifest principle to paired code paths. Effort S. Dedup: adjacent to the "two safeguards evolving independently" audit-lens candidate below (that = safeguard×safeguard state interaction; this = parallel-path guard inheritance) — cross-linked, distinct angle.

- **F6 — Living threat-model (ship-beat).** Pattern: the threat-model omits several shipped routes. Home: **ship beat** check (threat/security module) — a new route ⇒ a line in `docs/threat-model.md`, else the review is incomplete. Effort S. Pairs with F4.

- **F7 — Files outgrow readability (soft mechanical + architect fold).** Pattern: product files past ~1000–1400 lines, dense separator-banner ceremony. Loop-blind because the "cut it back when it outgrows" rule applies to the protocol's own core, not product code, and the architect persona is folded into Builder with no trigger. Homes: (a) **`## Setup` toolkit proposal** — a soft size-warning tool (threshold ~800), a signal not a block; (b) **`builder.md` architect fold** — on touching an over-threshold file, offer decomposition as an option (never a block). Effort M. Dedup: sibling of the "orchestrator length watch" entry (that = our own core); this = product code budget.

**Sequencing note:** F1/F2/F3/F5 are universal (code-kind), F4/F6 are security-surface-conditional (capability module), F7 is a quality-tool + architect-fold pair. The cheap mechanical wins (F2) and the cheap class-specific Reviewer items (F1/F3/F5) are the highest leverage-per-effort. Scope each through the loop when picked up; several Reviewer-checklist items could ship as one capability-module change.
