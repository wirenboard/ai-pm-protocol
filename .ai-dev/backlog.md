...
- **Installer npx-cache stale no-op probe.** `npx github:aadegtyarev/ai-dev-protocol` caches the github checkout and can silently re-install a STALE version (root of a multi-session false-"it's updated" confusion this day: a downstream sat on 5.17.3 while every "update" was a cache no-op, only resolved by deleting the vendored tooling / clearing `~/.npm/_npx`). Want: the installer detects a likely stale-cache no-op (e.g. compares the vendored version against the remote ref, offline-graceful) and warns/points at the cache-clear step. 5.17.7 documents the manual recipe (README `## Updating an existing install` + INSTALL.md `## Upgrade`); this would make it mechanical. Backlog (Operator deferred 2026-06-19).
...
## Installer self-verify of the Claude hook wiring — Fork B follow-up — 2026-06-19
