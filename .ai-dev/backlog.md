...
## Personal foreign-model re-bake dirties TRACKED agent files — clobber-on-update + accidental-commit leak — 2026-07-01 (Operator-reported, live)

**Symptom (hit live configuring this repo for cross-model).** For a subagent to route foreign, its `.claude/agents/<seat>.md` must bake the **bare tier alias** (`model: sonnet` → resolves via `ANTHROPIC_DEFAULT_SONNET_MODEL`); the committed/shared bake is the **concrete native** id, which does NOT route foreign. So a developer with a personal `config.local.json` binding must re-bake, which **modifies the tracked, committed agent files in place**. `config.local.json` itself is gitignored, but the agent files **cannot be** — they are tracked shared artifacts, and gitignore does not apply to tracked files. Documented today only as "don't commit them" (personal-multi-model-setup.md), i.e. discipline, not a mechanism.

**Two concrete risks the Operator named:**
1. **Clobber on update.** A protocol update / re-install / any re-bake regenerates those agent files from the shared config — silently **overwriting the personal bare-alias bake**, so the developer's foreign routing drops until they re-apply `config.local`. No warning.
2. **Accidental-commit leak.** The Orchestrator (or any `git add -A`, a teammate, a careless stage) can **commit the personal-baked agents into the shared repo**, forcing one developer's foreign model ids onto the whole team — the exact thing the personal/shared split exists to prevent. Today only the "stage named paths only" persona rule stands between this and a leak.

**Directions to scope (research/design first).** (a) A **personal-agents overlay** that Claude Code reads with higher precedence than the committed `.claude/agents/` (if the harness supports a per-user agent dir), so the personal bare-alias bake lands somewhere gitignored and the tracked files stay pristine. (b) Make the launcher apply the bare-alias routing **without** re-baking tracked files — e.g. a launch-time subagent-model override — if any harness mechanism allows it (`CLAUDE_CODE_SUBAGENT_MODEL` is global, not per-seat, so not a direct fit; needs investigation). (c) A **guard**: the installer marks personal-baked agents (a sentinel/`--skip-worktree` automation) and/or the merge-gate refuses a commit that stages a personal-bare-alias agent, so a leak is mechanically blocked, not discipline-held. (d) At minimum, the re-bake **warns** when it is about to clobber a personal bake, and the ship path warns if a personal-baked agent is staged. Its own feature + review; connects to the personal-multi-model-setup design (`docs/decisions/personal-multi-model-setup.md` "Honest limitation"). Operator-flagged.
...
## ai-dev consume: `/dev-setup` reads `modelpipe --list` and offers the proxy's routes per seat — 2026-06-30 (Operator-requested)

The CONSUME counterpart to the modelpipe expose item above. modelpipe `--list` (0.4.0) now emits a safe route-table summary (model ids + backend host + capabilities, no secrets), so the expose CLI surface the setup dialog needs already exists. **Build:** teach the orchestrator's setup model-per-seat dialog (`src/agents/procedures/setup.md` step 2) to, when a running proxy is reachable, run `modelpipe --list`, parse the configured model ids/capabilities, and **offer those concrete ids per seat** instead of asking blind — falling back to the typed-id ask when the proxy or its `--list` surface is unavailable (no hard dependency). setup.md already carries the intent line; this turns it into a real read. **Note the coupling:** lands right after `feature/haiku-model-allow` (the vanilla-vs-customized + haiku-allow change), which rewrites that same dialog bullet — sequence after it merges so the two don't collide. **Scope/safety:** consume only the safe surface `--list` exposes (never key values); the per-seat write still goes through the existing baked/launch split. Likely a `research`/design pass on the parse contract (the `--list` JSON shape) first, then the loop.
...
## WATCH: 3 moderate dev-only CVEs (markdownlint-cli2 transitive) — accepted — 2026-06-28

`npm audit`: js-yaml + markdown-it (transitive under `markdownlint-cli2`), a quadratic-DoS class over the repo's OWN trusted markdown — no untrusted-input path, fix needs a breaking `markdownlint-cli2` downgrade. Re-check on the next `markdownlint-cli2` major.
...
**Epic shipped (5.18.0–5.19.0):** the mechanical floor — trunk-push deny, `git add -A`/commit-on-main deny, default-ON pre-push quality hook — and the mandatory branch-protection + required-CI setup step (F2-1) are in. Decision: `docs/decisions/persona-floor-external-substitute.md`. One fast-follow open:
- **F2-2 (STILL OPEN)** — the agent runs the `gh api` branch-protection call itself on the Operator's confirm (a forge-mutating action; admin scope) instead of only printing the recipe. **Priority raised 2026-06-30 by a downstream incident:** F2-1's printed recipe was never applied with the CI-check, so the floor never existed — F2-1 alone is insufficient. (F2-1 itself shipped; sibling F-A — live-protection verification — RESOLVED 2026-07-01 via the `## Audit` branch-protection-live dimension.)
...
The multi-repo-components epic shipped (boundary mechanism 5.15.0 + coordination workflow 5.16.0; research `docs/decisions/multi-repo-components.md`). One item was consciously kept OUT and is its own future feature:
...
## Reference-class plan-time estimate from real ship history — 2026-07-02 (idea, Operator)

The plan-beat's estimate (`docs/decisions/estimation.md` buckets) is a human-effort-equivalent scope signal, not a wall-clock forecast for agent execution — a live 8D fix estimated at ~3.5–4.5h (Option-C scope: new merge function, new file convention, 8+ files) actually ran Builder-side in ~11 minutes. The Operator's objection: a scope bucket that's this far off isn't just mislabeled, it's not useful for a decision either. Candidate fix: stop guessing in human-hours and instead ground the estimate in **this repo's own ship history** (reference-class forecasting) — "shaped like feature X, which took N spawns / M wall-time / K tokens" — rather than an unanchored guess.

**The gap that blocks it today:** the ship-time cost relay (Orchestrator's `## Your seat` — "spawns, wall time, token spend") is already computed at every ship, but it is spoken once in chat and never persisted; there is nothing durable/queryable for a future Planner to reference against. Building this needs: (1) a small durable log (one line per shipped feature: version, one-line shape, spawn count, wall-time, tokens) written by the Orchestrator at ship — a new writer, not a repurposed one (the state pointer explicitly "points, never journals," so this is a new, different artifact, not an addition to `.ai-dev/state/current.md`); (2) a Planner-side read/reference step at estimate time, matching the new feature's shape against the log's closest precedent. Genuinely a small feature (new log format, new writer, new reader), not a one-line doc tweak — needs its own plan before building. Not yet built, not yet planned — capture and revisit when there's room for it.
