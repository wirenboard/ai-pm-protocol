...
## BUG: models-per-role setup dialog is too coarse — no independent per-seat model pick — 2026-06-30 (Operator-reported)

**Symptom (Operator screenshots).** Running `/dev-setup`, the "Модель для ролей" step offers only coarse presets: (1) inherit all (orchestrator+builder → session, reviewer → its pin), (2) pin ONE id for orchestrator+builder TOGETHER, (3) change the reviewer pin, (4) type something. There is **no path to set builder ≠ orchestrator**, nor to pick each of the three seats independently. The Operator asked "where do I choose the builder's model? the reviewer's? the orchestrator's?" — and the dialog doesn't let them, one seat at a time.

**Gap.** The config supports full per-seat granularity (`roles.{orchestrator,builder,reviewer}.model`, each independent), but the SETUP DIALOG (`src/agents/orchestrator.md` `## Setup` step 2, models-per-role) collapses orchestrator+builder into one bundled choice and offers presets instead of a per-seat picker. So the advertised "model per role" is under-surfaced — a user can't reach the config's real granularity through the dialog.

**Fix direction (ONE slice — Operator-scoped 2026-06-30).** Two coupled UX fixes, shipped together:
1. **Per-seat dialog.** Make the models-per-role step ask **three independent questions** (orchestrator / builder / reviewer), each offering session / auto / a typed concrete id, defaulting to the current value. Keep the zero-config lead (auto/session recommended) but allow each seat to diverge. Honesty caveats unchanged (Claude bakes per-seat model:; auto resolves to an Anthropic id — flag for external-proxy users who need a concrete proxy-routed pin).
2. **The orchestrator must KNOW a model change needs a re-bake — and give the context-correct command.** Today nothing tells the operator that editing `roles.*.model` (via dialog OR by hand) requires re-assembling the agents so the baked `model:` updates (Claude bakes per-seat). The orchestrator should, on ANY per-seat model change, **re-apply the config automatically** (Setup step 4 already does this when SETUP runs it — extend it to a standalone model-change too) AND surface the **right install command for the context**: dogfood (`node src/adapter/install.mjs . --dogfood --platform <p>`) vs downstream (`node .ai-dev/tooling/src/adapter/install.mjs . --platform <p>`, NO `--dogfood`). The live trigger: the Operator ran the DOGFOOD command in a downstream repo (`noos`) and hit MODULE_NOT_FOUND because `src/adapter/install.mjs` exists only in the protocol source — the orchestrator never told them the downstream path. A `mode/profile-switch`-style lightweight "model switch mid-stream" handler (mirroring orchestrator.md `## Setup` *Mode switch mid-stream*) that flips the pin + re-bakes + announces is the natural home.

**Workaround until fixed:** edit `roles.*.model` directly in `.ai-dev/config.json`, then re-run the CONTEXT-CORRECT installer — downstream: `node .ai-dev/tooling/src/adapter/install.mjs . --platform claude` (no `--dogfood`); the protocol source repo: `node src/adapter/install.mjs . --dogfood --platform claude`.

## BUG: installer falsely logs "wrote .ai-dev/config.json (minimal default)" on every run — 2026-06-30 (Operator-reported, honesty/log)

`ensureConfig` (`install-core.mjs:81-96`) writes the default config **only where absent** (`if (fs.existsSync(dest)) return;` — correct, never clobbers). But the install summary (`install.mjs:201`) prints `• wrote .ai-dev/config.json (minimal default — run /dev-setup to configure)` **unconditionally**, even when ensureConfig was a no-op (config already present). So a downstream re-install on a configured project prints "wrote config" though it touched nothing — which **read as a config-clobber scare** to the Operator (they thought their freshly-set model pins / mode / profile / team+forge had been overwritten). No data loss (the early-return protects it), but the log lying is a real honesty defect — the install summary must report what it ACTUALLY did. **Fix:** `ensureConfig` returns whether it wrote (or the caller checks `fs.existsSync` before); gate the summary line — print "wrote … (minimal default)" only on an actual write, else "kept existing .ai-dev/config.json" or nothing. Trivial; fold into the multi-model UX-fix slice or a fixup.
...
Today multi-model routing is a manual flow (write `.ai-dev/model-routes.json` by hand, export keys, launch via the wrapper script) — nothing in Claude Code's own surface configures or starts the proxy. Wanted: (a) a `/dev-setup`-style **dialog** that asks the Operator the cross-endpoint routing (providers, model→provider, keys) and **generates** `.ai-dev/model-routes.json`; (b) **auto-start** the proxy at the beginning of each session. (b) is the hard part — Claude Code has no per-session pre-launch hook that can set `ANTHROPIC_BASE_URL` for its OWN process (the env must be set BEFORE claude starts; the launcher wrapper exists precisely because of this). Investigate whether a SessionStart hook / settings env injection can wire `ANTHROPIC_BASE_URL` for the running session, or whether the wrapper-launcher remains the only honest mechanism (then the deliverable is making the wrapper frictionless + documented, not a true in-harness autostart). Pairs with the submodule task above.
