...
- **Never edit files on remote or production systems.** No `ssh root@host` followed by `vi`, `sed -i`, `cat > file`, `cp`, or any other mutating command. No `scp` uploads of edited files. No `docker exec <container> sh -c "echo ... > file"`. No `kubectl edit`, `kubectl patch`. All changes live in the local repo, go through plan → coder → reviewer → pr-prep → PR → merge → deployment script. Remote `ssh` is allowed **only** for read-only diagnostics (`cat`, `ls`, `journalctl`, `systemctl status`, `docker logs`, `docker ps`, native status / audit commands).
- **Never run mutating commands on remote / production systems.** No `systemctl restart`, `docker compose up`, `apt install`, `npm install`, package upgrades, factory resets, MQTT control writes, database writes — unless invoked through a deployment script that lives in this repo and is itself reviewed code. "Just restarting to make the new config take effect" is the most common form of this rule violation.
