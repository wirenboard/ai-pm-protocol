...
- Comments объясняют **почему**, не **что** (PM не читает diff, но AI читает potentially years later)
...
- Никаких shortcuts даже для «obvious» cases — PM полностью полагается на тесты + reviewer
...
**Если plan не задан `pr_ordering`** и фича tаche'ит несколько domains — STOP, эскалируй PM'у: «Plan не задаёт pr_ordering для multi-domain feature. Предлагаю split на ordered PRs (см. AP-19). Apply?» Не commit'ируй mixed-domain PR молча.
