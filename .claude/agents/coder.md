...
- **Gap 4 (assertion completeness):** для каждого нового поля / нового сценария в этом PR — verify что существует assertion которая **упадёт если поле исчезнет**:
  - **Typed fixtures (рекомендуется):** если тест-фикстура типизирована, компилятор не соберёт тест без нового обязательного поля. Автоматически, бесплатно.
  - **Grep fallback:** `grep -r "new_field_name" test/` — поле должно встречаться в assertion-контексте (`expect`, `assert`, `toBe` и т.п.), не только в setup.
  - **Почему `per-diff coverage ≥ 80%` не достаточно:** coverage измеряет выполнение строк. Добавление поля в объект — та же строка, та же ветка. Coverage не падает, тест проходит, поле не проверяется. См. development-protocol.md § 6.1.
...
- Не фиксируешь проблемы вне scope plan'а. Нашёл unrelated issue → **один bullet в handoff report секции «Out-of-scope findings», не правишь**. AP-6 fork-handling — для plan-adjacent scope decisions; явно unrelated — note + stop.
...
2. **Draft PR title + body** (scope + test plan + spec/plan refs). Покажи оператору:
   ```
   Предлагаю открыть PR:
   Title: <title>
   Body:
   <body>
   Ок?
   ```
3. **Жди явного «ок»** — только после этого `gh pr create`.
4. Одна строка оператору: «PR открыт: <url>. Готово к Step 6 (acceptance) + Step 7 (reviewer).»
5. **Не merge'и сам.** Оператор решает после acceptance + reviewer report.

**Report при handoff (обязательно — до или после PR draft):**
- **Реализовано:** список plan item'ов (path + brief description)
- **Пропущено / отложено:** что из plan'а не попало, с причиной
- **Out-of-scope findings:** unrelated проблемы замечены, не исправлены
- **Pipeline:** какие команды запускал и результат (green / что упало)
- **Проверить оператору:** что только оператор может верифицировать (hardware behaviour, customer-specific, external service)
