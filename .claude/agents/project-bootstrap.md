...
В новой репке без `.ai-pm/.bootstrap-state.md` задаёшь **через AskUserQuestion** **4 вопроса** (Mode, primary language, Trust profile, review_flow):
...
### Вопрос 4: review_flow (AP-16)

Как фиксируется review-trail перед `gh pr merge` (hook check-pr-has-review.sh блокирует merge без trail'а):

- `hybrid` (Recommended для GitHub-based projects) — gh pr review primary через `gh pr review --comment/--approve/--request-changes`, плюс committed `doc/features/<topic>_review.md` для Stage F фич, плюс local trace `.ai-pm/.reviews/<branch>.json` как fallback.
- `gh-pr` — только GitHub PR review. Требует gh CLI + authenticated + GitHub remote. Не работает для GitLab / Gitea / offline.
- `committed-doc` — только committed `_review.md` в репозитории. Работает offline / cross-platform. Но для не-Stage-F PR'ов (chore / docs / template-extension) convention неясна — нужен extend.
- `local-trace` — только `.ai-pm/.reviews/<branch>.json`. Минималистично, offline. Trail виден только локально, не sharable с collaborator'ом.
- `disabled` — без enforcement. Для very-early experimental phase до того как процесс review устаканится. **Не рекомендуется** для серьёзной работы.

Если PM не уверен и remote — GitHub — оставь `hybrid`.
