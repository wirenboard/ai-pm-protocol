...
| **«ревью PR / проверь код»** | Invoke `reviewer` subagent для текущей feature-branch. Reviewer обязателен ДО `gh pr create` / `gh pr merge` (AP-16 verdict-gate hook блокирует без review-trail). Не Stage F branch → trail через local `.ai-pm/.reviews/<branch>.json` или committed `doc/reviews/<branch>_review.md`. Verdict `request-changes` блокирует — fixes apply'ятся первыми, потом перезапуск reviewer'а. Override через `[review-override: <reason>]` — explicit operator decision, не AI default. |
