...
**Исключение — exported dead code:** `noUnusedLocals` в TypeScript (и аналоги) ловит внутренние символы, но **не** `export`-нутые публичные API с нулём вызывающих снаружи модуля. Если diff добавляет/изменяет `export`-нутый символ — grep по имени в остальных файлах репо; ноль матчей → `[nit]` finding. Scope: только символы из текущего diff.
...
- Polling loops в тестах (`setInterval` / `setTimeout` / `while+sleep` для ожидания результата) — предпочтительнее event-based wait; polling утекает при timeout → `[nit]` finding
...
**Исключение:** если spec/plan явно устанавливает single-process constraint — downgrade до `[nit]`.
...
- Grep по diff: есть ли validation / parse / schema check перед первым использованием?
- Нет → `[blocking]` finding: «external input X используется без валидации — malformed value вызовет unhandled exception».

**Scope:** только новые input-пути из текущего diff. Не поднимай pre-existing unvalidated paths.
...
- Grep по diff: есть ли в logic-коде строковые конкатенации / template literals формирующие structured names?
...
- Grep по ключевым именам / паттернам из diff'а в остальных файлах репо.
- Grep подтвердил совпадение (≥ 3 строки идентичной структуры) → `[blocking]` finding: «идентичная логика уже есть в `<file>:<line>` — извлечь в shared helper».
- Grep не подтвердил → не флагируй.
