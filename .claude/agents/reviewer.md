...
**Исключение — exported dead code:** `noUnusedLocals` в TypeScript (и аналоги) ловит внутренние символы, но **не** `export`-нутые публичные API с нулём вызывающих снаружи модуля. Для каждого `export`-нутого символа из diff'а — **один grep по точному имени** в остальных файлах репо; ноль матчей → `[blocking]` finding. Scope: только символы из текущего diff.
...
- Если diff добавляет новый event handler, pub/sub callback, async flow или state transition path — проверь: хотя бы один тест явно активирует этот путь? Если нет → `[blocking]` finding «новый behaviour path без теста»
- Polling loops в тестах (`setInterval` / `setTimeout` / `while+sleep` для ожидания результата) вместо event-based wait → `[blocking]` finding: «polling в тесте утекает при timeout — заменить на event-based wait»
...
### 3.7. Behavioral correctness floor (event-driven код)

**Trigger:** diff содержит event handler'ы, pub/sub callbacks, file watcher'ы или reactive подписки.
**Scope:** только паттерны добавленные/изменённые в текущем diff.
**Ground truth:** не требует `spec_reference` — это runtime safety invariants (аналогично §3.4 Security).

#### 3.7.1. Feedback loop (emit→handler→re-emit)

Handler на событие X публикует/эмитирует то же событие X (или цепочку, ведущую обратно к X)?
- Если да — есть ли guard (флаг, origin check, old/new value comparison)?
- Нет guard → `[blocking]` finding: «handler на X может re-publish X без guard — потенциальный echo loop».

#### 3.7.2. Self-triggering observer

Write/save callback модифицирует тот же файл/ресурс, который наблюдается?
- Если да — есть ли debounce или origin check?
- Нет → `[blocking]` finding: «save() может re-trigger собственный watcher — нужен debounce или origin check, либо явный комментарий почему безопасно».

#### 3.7.3. Process-global mutable state

Diff мутирует module-level или process-global объект (singleton, global registry, `process.env`, shared config)?
- Безопасно ли при повторной инициализации или нескольких инстансах?
- Нет документированного constraint → `[blocking]` finding: «глобальный singleton мутируется без документированного ограничения "single instance only" — добавить комментарий или защитную проверку».

**Исключение:** если spec/plan явно устанавливает single-process constraint — finding не нужен (constraint задокументирован).

#### 3.7.4. Subscription without cleanup

Diff добавляет event subscription / watcher / listener без соответствующего cleanup (`.off()` / `removeListener()` / `.close()` / `unsubscribe()`)?
- Есть ли teardown / dispose / destroy path где subscription снимается?
- Нет cleanup → `[blocking]` finding: «подписка на X без cleanup — memory/handle leak; добавить в teardown/dispose».

### 3.8. Heuristic code quality floor

**Trigger:** любой diff с production-кодом (Tier 3).
**Ground truth:** не требует `spec_reference` — universal correctness patterns.

#### 3.8.1. Input validation at system boundaries

Функция принимает внешний input (MQTT message, HTTP request, file content, env variable, user input) и использует его без валидации?
- Читай только diff — есть ли в той же функции/методе validation / parse / schema check перед первым использованием входного значения? Это чтение diff'а, не сканирование репо.
- Нет → `[blocking]` finding: «external input X используется без валидации — malformed value вызовет unhandled exception».

**Scope:** только новые input-пути из текущего diff. Не поднимай pre-existing unvalidated paths. **Ноль дополнительных grep-вызовов** — всё видно в diff'е.

#### 3.8.2. Logic vs. builder separation

Inline string templates, hardcoded resource paths, format strings или protocol-specific patterns (MQTT topics, URL patterns, SQL fragments) embedded прямо в бизнес-логике?
- Читай diff: есть ли в logic-коде строковые конкатенации / template literals формирующие structured names? Это чтение diff'а, не сканирование репо.
- Да → `[blocking]` finding: «inline builder в логике — вынести в отдельный builder/constant».

#### 3.8.3. Duplication

Diff вводит логику, которая уже существует в других модулях проекта?
- Для каждой новой функции / метода / хелпера из diff'а — **один grep по точному имени символа** в остальных файлах репо.
- Grep нашёл идентичную реализацию (≥ 3 строки совпадающей структуры) → `[blocking]` finding: «идентичная логика уже есть в `<file>:<line>` — извлечь в shared helper».
- Grep не нашёл → не флагируй.
