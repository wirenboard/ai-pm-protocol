...
- **Expected-discipline gap (semantic doc-migration on template bump)** — the content-discipline sibling of the structural `### Pending-migration detection` sweep. For each discipline listed in **`### Expected-discipline manifest` in `MIGRATIONS.md`** — referenced **by name**, never re-list the disciplines or their checks here — evaluate the manifest entry's own applicability test and satisfied-check against the project's current docs:
  - The discipline does **not** apply (per its applicability test — e.g. `docs/threat-model.md` absent for the threat-model discipline; zero user-facing features for the journeys / product-story disciplines) → **silent**.
  - For the journeys / product-story applicability test, **reuse the dimension-1 human-role-subject extraction result** (any merged feature whose scenario-1 subject is a human role) — do **not** re-extract.
  - The discipline applies **and** its satisfied-check shows the gap (the manifest phrases each as a positive-presence-of-a-gap test — `<placeholder>` present / empty table / skeletal funnel) → **note**. Applies and satisfied → **silent** (idempotent: a project that already satisfies the discipline produces zero findings on re-run).
  - **Presence-only — never prose-police the discipline's content.** The check matches the manifest's structure/placeholder test, never whether the threat-model, journey, or value story is *good*; a populated-but-"weak" artifact is satisfied (silent). The PM owns meaning.
  - Remediation: spawn `pm-architect` (the owner) after the orchestrator relays the discipline's foundational questions — the manifest entry names the question source (`### Foundational product questions` in `WORKFLOW.md` or the bootstrap threat-model Q-set). (The threat-model *skeleton* case is **already blocking** under the `docs/threat-model.md` sub-check above; this entry rides that existing rule and does not weaken it.)
