...
- **Journey identifier-restatement** (sibling of the System-invariants index check above; same structural family) — surface is **`docs/user-journeys.md` ONLY** (step bodies and `**Invariants:**` blocks). It is the journeys-surface complement of the contract / product-map structural-token note (dimension 4 above and the product-map value-line note): non-overlapping by **file** (this check never reads contracts or the product map), so no token can fire on both, and it does **not** re-encode that note's wire-token list — it points at it. **Presence-conditional:** silent when `docs/user-journeys.md` is absent, carries no backticked / fenced declared-taxonomy token, and carries no wire-token shape. When the file is present:
  ...
  - **(sc2) Wire-token SHAPE in a journey → note.** A journey step body / `**Invariants:**` block carrying a wire-token shape — **the wire-token shapes the structural-token note above defines** (dimension 4, the contract / product-map note); reuse that vocabulary **by reference**, do not re-encode the list — inherits that note's domain-vocabulary exclusion (`DimmableLight` / `Matter` / `fabric` never flagged). → **note** (machine identifier restated in a journey).
