...
9. **Cross-model review** — neutral, default session. Ask: "Plan and code run on your session model. Review and audit can run on a **different** model for independent blind spots — keep them on the session model (default), or set a cross-model (`auto` / `opus` / `sonnet`)?" Default **session** (today's behavior) if the PM skips or abstains. See `### Cross-model review` in `workflow/review-typology.md` (the single source of the enum, the `absent file OR unrecognized ⇒ session` default, and the Haiku-blacklist — do not re-state them here).
...
- `.ai-pm/review-config.md` — write from Q9 (the cross-model answer): the three model settings `review-diff-model` / `review-full-model` / `audit-model` (default `session`) + `review-scope: auto`. See `### Cross-model review` in `workflow/review-typology.md` for the keys, the enum, the `absent file OR unrecognized ⇒ session` default, and the Haiku-blacklist — do not re-encode them here. Created at bootstrap, but no consumer may *require* it: its absence elsewhere ⇒ `session` (so every older project is unaffected and no migration is introduced). When Q9's answer is a single cross-model (e.g. `opus`), set all three model settings to it unless the PM scoped a specific setting.
