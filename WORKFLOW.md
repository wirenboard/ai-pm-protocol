...
- **Use only the designated `pm-*` agent for each role — never a role substitute, from any source.** Spawn only the protocol's `pm-*` agents (in the adapter directory) and run only the `pm-*` commands; never fill a protocol seat with a substitute, whether a similarly-named `wb-*` role duplicator (from an installed toolset) **or a generic harness built-in** (`general` / `build` / `plan` — a generic carries none of the role's discipline). Each pipeline role has exactly one designated filler (the role→agent table in `workflow/roster.md`); a generic standing in for it is the **same** violation as a `wb-*` duplicator. Both sources are denied by the enforcement layer's **named** deny-list (realized per harness — see the harness-reference table); the built-in **engines** the protocol delegates to on purpose (`code-review`, `deep-research`) and `wb-*` *knowledge* skills are **not** gated — they are the designated fillers / aids, not substitutes. Full roster + enforcement mechanics: `workflow/roster.md` and `workflow/enforcement.md`.
...
- **Delegation & gate integrity.** The orchestrator drives the pipeline but **never produces, paraphrases, reuses, or skips an autonomous agent's deliverable** by any route — not by editing its canon, not by skipping its gate, not by substituting a crashed agent's output, not by re-presenting a stale on-disk artifact (reading an agent's artifact for context — Step 1 docs, an existing plan or review — is fine; the banned move is *presenting* it as this turn's gate result, not reading it). **A gate is satisfied only by a fresh spawn of the owning agent this turn**; an artifact already on disk is evidence of a *prior* run, never a substitute for this one — **failed / missing / already-existing / skipped all count as "not run".** When an agent's canon must change, respawn that agent (the **edit-route instance** of this rule). The only things the orchestrator legitimately produces are the **outputs of the processes it drives** — the backlog, recorded PM decisions, the gated Pass-2 `## Code review` trail, the gated advocate `## Resolutions` trail, protocol-gap reports, and git ops. Full rule, the two carve-outs, the artefact list, the fresh-spawn-this-turn test, and the named-rationalization ban: `workflow/enforcement.md`.
...
- **Durable-text frugality.** Authored durable text (docs, code comments, commit messages) is **reader-first: plain-language, fact-first, current-state-only, provenance-as-pointer, distilled-not-accumulated** — supersede-don't-edit a changed decision, comment why-not-what, one purpose per unit. Full rule (the tombstone form, comment-restraint definition, hard-cap set, the closed enum of 4 graduation targets): `workflow/doc-style.md`.
...
| Boundary / delegation & gate integrity (incl. its edit-route instance) / remote-system **full** rules + carve-outs + hook-level enforcement | `workflow/enforcement.md` |
...
| `## Durable-text frugality` (fact-first/BLUF, current-state-only, supersede-don't-edit + tombstone, provenance-as-pointer, one-purpose-per-unit, comment-restraint, numbers-as-targets, the closed enum of 4 graduation targets) — the **frugality** home for the durable-text family {docs, comments, commits}; the **legibility** sibling (read-before-ship clarity) stays in `workflow/pm-comms.md` | `workflow/doc-style.md` |
