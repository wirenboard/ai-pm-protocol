...
This file is a **thin constitution + router**. It carries the cross-cutting invariants that must hold on *every* action, the Step 0–7 pipeline skeleton, and a navigation map to the on-demand **`workflow/*.md`** topic files. Each topic file is the single home for its rule; read it with the Read tool at the step that needs it (the `@`-import loads only this thin core — the topic files are read on demand, not eager-loaded). The section name is the stable anchor: a rule referenced as `` `### X` in `workflow/<topic>.md` `` resolves to the `### X` heading in that file.

---

## Cross-cutting invariants (always on)

These hold on **every** action, regardless of which pipeline step you are in. The full detail of each lives in a topic file (cited inline); the one-liner here is the always-true subset.

- **Use only the `pm-*` agents, never the `wb-*` role duplicators.** Spawn only the `.claude/agents/pm-*` agents and run only the `.claude/commands/pm-*` commands; never substitute a similarly-named `wb-*` role agent or role skill that occupies a protocol seat. This is hook-enforced (a `PreToolUse` deny-list in `.claude/settings.json`) — built-in engines (`code-review`, `deep-research`) and `wb-*` *knowledge* skills are not gated. Full roster + hook mechanics: `workflow/roster.md` and `workflow/enforcement.md`.
- **Project boundary.** Every agent stays within the project root (`git rev-parse --show-toplevel`); never read, search, or write outside it — no parent directories, no sibling repos. Full rule: `workflow/enforcement.md`.
...
- **Read before apply.** Before applying or citing any `workflow/*.md` rule — in your own reasoning or when spawning an agent — Read that topic file in the current turn; never act on it from memory, because this always-loaded core carries only the router pointer + one-liner and the load-bearing detail lives in the topic file.
...
The PM makes product decisions and does not read code. Always: **lead with user impact** (not code), **explain decisions as trade-offs**, **ask one question at a time** (2–3 concrete options, recommend one), **surface substantive forks via the AskUserQuestion tool** (simple proceed/confirm gates stay in prose), **never show code**, **no jargon without a plain-language gloss**. Full rule list + ✓/✗ examples + the autonomous-mode rider: `workflow/pm-comms.md` (worked examples: `workflow/examples.md`).
...
- **Step 3.5 — Product-readiness gate** (user-facing features only): spawn `pm-product-advocate`; `clean` → pass, `gaps: N` → one `AskUserQuestion` pass (or the autonomous derive-or-escalate branch). → `workflow/pipeline.md`
