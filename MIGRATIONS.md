...
- **Code-review-trail loud-marker normalization — empty placeholder → `## Code review: NOT YET RUN`.** A downstream review file whose `## Code review` section is the pre-vX empty placeholder is normalized so the now-loud `pm-auditor` dimension-1 rule can act on it. Detection: see `### Pending-migration detection` above (unstamped code-review trail). When it applies:
  ...
  **This is a text-only normalization performed by the orchestrator** — the trail's owner, per the `WORKFLOW.md` edit-ownership carve-out (the Pass-2 `## Code review` trail is the single review section the orchestrator writes). It does **not** assert that review ran; it only makes the latent gap **visible** by replacing a deceptive-empty heading with the loud marker.
