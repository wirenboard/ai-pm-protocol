...
- **Unstamped code-review trail (loud-marker not applied):** an existing downstream `.ai-pm/reviews/<topic>_review.md` whose `## Code review` section is the pre-vX **empty placeholder** — detect by the **positive presence** of an **unstamped** `## Code review` heading: a `## Code review` line that carries **no** `— passed` date **and** is **not** the `## Code review: NOT YET RUN` marker (e.g. the old empty `## Code review` heading, or the old `## Code review` / `(updated by orchestrator …)` placeholder text). A section already stamped `## Code review: <date> — passed`, or already reading `## Code review: NOT YET RUN`, is **not** flagged. A review file with no `## Code review` section at all (a trivial-fixup `fixup-*_review.md`) is also not flagged. The remediation is the **code-review-trail loud-marker normalization procedure** below — a text-only normalization performed by the orchestrator (the trail's owner, per the edit-ownership carve-out in `workflow/enforcement.md`).

- **Missing `.ai-pm/review-config.md` (cross-model review default applies):** an existing downstream project with **no** `.ai-pm/review-config.md` file. Per the `absent file OR unrecognized ⇒ auto` default in `### Cross-model review` in `workflow/review-typology.md`, an absent file resolves to `auto` — **review and audit begin running on a model different from the session** (cross-model). This **is** a behavior change, stated honestly — but **non-destructive** (only *which model* reviews changes; review still runs and the `## Code review` stamp gate is untouched), **announced** at the first review ("Code review on Sonnet…"), and **reversible** with one line (`review-*-model: session`). It is **not an interactive offer** — there is no setup nudge, no opt-out-state to record, no re-nag; the default simply applies and is documented. A project that wants the old session-model behavior writes `review-*-model: session`. Detect by the **absence** of `.ai-pm/review-config.md`; a project that already has the file is **not** flagged. The remediation note is the **cross-model review default-change note** below.

- **Stale `process` project-kind line (documentation rename not applied):** an existing downstream `CLAUDE.md` carrying a `## Project kind: process` line — the kind value an early v2.18 adopter declared, before the v2.18 `process` flavor was generalized to `documentation` (`### Project kind` in `workflow/project-kind.md`). Detect by the **positive presence** of the literal `process` value on the `## Project kind:` line. A line reading `software` / `documentation`, or an absent line, is **not** flagged. This is the durable greppable anchor that makes the `absent OR unrecognized ⇒ software` default non-silent: until migrated, the stale line falls safely to the software default (no break), and this flag nudges the one-line rename so the adopter regains the documentation flavor. The remediation is the **`## Project kind: process` → `documentation` procedure** below — a trivial, mechanical single-line rename performed by the orchestrator (owner of `CLAUDE.md` at runtime); no other artefact moves.

- **`state-archive-home` not superseded (still on the curated-archive convention):** an existing downstream still on the persist-archive-on-branch model — detect by the **positive presence** of either a tracked `.ai-pm/state/archive/` directory **or** a `state.md` whose on-finish instruction still curates a completed snapshot into that directory (the archive-on-completion convention). A project whose `state.md` resets to idle on finish and keeps no archive directory is **not** flagged. The remediation is the **`state-archive-home` supersede procedure** below — the convention stops; **no archive content is deleted** (git retains every prior snapshot incidentally).

- **Append-model standing docs (doc-frugality not applied):** an existing downstream still on the append-and-accumulate doc model — detect by the **positive presence** of either accumulated per-feature evidence files (`.ai-pm/reviews/<topic>_review.md` / `.ai-pm/arch/<topic>_arch.md` for already-merged features) that should have distilled to git under the O(1) model, **or** a standing doc carrying accreted decision history (an inline `Supersedes` / `corrects-the-earlier` / `[x] Resolved (date)` narrative the supersede-don't-edit form replaces). A project whose standing docs are lean (current-state-only, tombstone-on-supersede) and whose merged features carry no surviving per-feature evidence is **not** flagged. The remediation is the **doc-frugality migration procedure** below — a **distill-not-delete** performed per `### Graduation targets` in `workflow/doc-style.md`; every durable bit graduates to its home before its process evidence collapses to git.
...
- **Question source:** the bootstrap **threat-model Q-set** (the Q7 security answers named in `### Threat-model lifecycle` in `workflow/security-surfaces.md`).
...
- **Question source:** `### Foundational product questions` in `workflow/foundational-questions.md`, **per-feature tier** (the value/usability/scope-boundary set).
...
- **Question source:** `### Foundational product questions` in `workflow/foundational-questions.md`, **bootstrap tier** (discovery / onboarding / value / viability).
...
- **Code-review-trail loud-marker normalization — empty placeholder → `## Code review: NOT YET RUN`.** A downstream review file whose `## Code review` section is the pre-vX empty placeholder is normalized so the now-loud `pm-auditor` dimension-1 rule can act on it. Detection: see `### Pending-migration detection` above (unstamped code-review trail). When it applies:

  **This is a text-only normalization performed by the orchestrator** — the trail's owner, per the edit-ownership carve-out in `workflow/enforcement.md` (the Pass-2 `## Code review` trail is the single review section the orchestrator writes). It does **not** assert that review ran; it only makes the latent gap **visible** by replacing a deceptive-empty heading with the loud marker.
...
- **`## Project kind: process` → `documentation` rename — stale early-v2.18 kind line corrected.** A downstream `CLAUDE.md` carrying the early-v2.18 `## Project kind: process` line (declared before the `process` flavor was generalized to `documentation`) is brought onto the current kind vocabulary. Detection: see `### Pending-migration detection` above (a `## Project kind:` line carrying the literal `process` value). When it applies:
  ...
  **This is a trivial, mechanical single-line rename performed by the orchestrator** — the runtime owner of the downstream `CLAUDE.md` (the project's own file, not an agent-owned canonical doc). No other artefact moves; the `process` flavor and the `documentation` flavor are the same kind under two names, so the rename only updates the declared label, never the project's behavior.
  ...
  3. Back-compat is safe even before the rename runs: by the `absent OR unrecognized ⇒ software` default in `### Project kind` (`workflow/project-kind.md`), the stale `process` line already falls safely to the software flavor (no break) — the rename restores the project's correct declared kind (`documentation`) and silences the detection.

- **Cross-model review default-change note — absent `.ai-pm/review-config.md` → the `auto` default applies (documented, not offered).** A downstream project with no `.ai-pm/review-config.md` picks up the `auto` default on bump: review and audit begin running cross-model. Detection: see `### Pending-migration detection` above (absent `.ai-pm/review-config.md`). When it applies:

  **This is a documented default change, not an interactive setup-offer.** By the `absent file OR unrecognized ⇒ auto` default in `### Cross-model review` in `workflow/review-typology.md`, an absent file now resolves to `auto`, so review/audit run on a model different from the session. There is **no setup nudge, no opt-out-state to record, and no re-nag** — the default simply applies and is documented (here, the CHANGELOG, and `doc/architecture.md` `### Cross-model review`).

  - **Non-destructive.** Only *which model* runs review/audit changes — review still runs, and the `## Code review` stamp gate is untouched.
  - **Announced.** The orchestrator announces the model at the first review ("Code review on Sonnet…"), per the per-review announce in `### Cross-model review` — so the change is visible the first time it takes effect, not silent.
  - **Reversible in one line.** A project that wants the old session-model behavior writes `review-*-model: session`.

  Nothing to run here: no `git mv`/`git rm`, no offer, no idempotency state. The note exists so a downstream PM bumping the template understands why review now runs on a different model and how to revert.

- **`state-archive-home` supersede — the curated-archive convention is dropped (move-not-delete).** A downstream still on the persist-archive-on-branch model is brought onto the O(1) distillation model: a completed feature's state no longer becomes a curated snapshot under a per-topic archive directory — the state resets to idle on finish, and git retains the prior snapshot incidentally. Detection: see `### Pending-migration detection` above (`state-archive-home` not superseded). When it applies:

  **This is move-not-delete — no snapshot is removed.** Any snapshot the old convention already produced stays exactly where git put it (git history retains every byte). The migration only **stops the convention going forward**; it never deletes accumulated archive content, and it never strands a durable fact (durable facts have already graduated into the four living-reference homes — `### Graduation targets` in `workflow/doc-style.md` — so the snapshot was never the only copy). The model this supersedes is named once on purpose below; that naming is the supersede statement, not a live assertion of the old model.

  1. **Stop the convention (orchestrator).** Update the downstream `state.md` on-finish instruction: state resets to idle when a feature finishes; the prior snapshot is no longer copied into a curated per-topic archive directory. If a tracked archive directory exists, leave its existing content in place (git history) and remove only the **instruction** that keeps adding to it — never `git rm` the accumulated snapshots.
  2. **Idempotent.** A `state.md` that already resets to idle on finish, with no archive-on-completion instruction, is left untouched. Running the supersede twice is a no-op.
  3. Tell the PM in plain language: "Finished features no longer leave a saved copy of their progress notes in a project folder — the progress tracker just resets, and git already keeps the history. Anything important from a finished feature already lives in your architecture / contracts / backlog / stack notes. Nothing was deleted."

- **doc-frugality migration — standing docs distilled to lean form, per-feature evidence collapses to git (distill-not-delete).** A downstream still on the append-and-accumulate doc model is brought onto the distillation-engine model: each standing doc is compacted to lean, current-state-only form (the doc-style discipline), accumulated per-feature evidence collapses to git under the O(1) lifecycle, and the standing-doc set gains the doc-style guidance comment + the generated-section fences (the lean template set). Detection: see `### Pending-migration detection` above (append-model standing docs). When it applies:

  **This is distill-not-delete — every downstream guarantee is preserved.** A durable bit is **graduated into its home BEFORE its process evidence is dropped** (graduation per `### Graduation targets` in `workflow/doc-style.md`). Nothing is lost; durable knowledge only relocates from scattered evidence into the living reference, and the process narrative that justified reaching it collapses to git. The C.9 git-aware graduation check (auditor) and the D.13 pre-ship merge gate are the safety net that proves no durable bit evaporated unrelocated.

  This migration relocates content; it does not touch code. It is performed **by `pm-architect`** for the standing docs it owns (`docs/architecture.md`, `docs/user-journeys.md`, the authored `docs/product.md`, `docs/threat-model.md`) and by the orchestrator for the evidence-file collapse it owns.

  1. **Graduate first, then collapse.** For each already-merged feature whose evidence still survives as a per-feature file (`.ai-pm/reviews/<topic>_review.md`, `.ai-pm/arch/<topic>_arch.md`): confirm each durable bit it carries is present in its graduation home (`### Graduation targets`) — a decision in an `architecture.md` decision record, a contract in the contract registry, a deferred finding in `.ai-pm/backlog.md`, a stack rule in `docs/stack-notes.md`. Any durable bit not yet in its home is **graduated there first**. Only once every durable bit is in a durable home does the surviving evidence file become collapsible to git (it is no longer the single copy of anything). **Mixed-project safe:** presence-in-any-durable-home satisfies graduation, so a project mid-migration (some features evaporated, some not) is never false-blocked.
  2. **Distill the standing docs (pm-architect).** Compact each owned standing doc to lean, current-state-only form: a changed decision becomes a supersede-don't-edit tombstone (no accreted `Supersedes` / `corrects-the-earlier` / `[x] Resolved (date)` history in the live doc — that narrative is git), provenance becomes a one-line pointer, and the doc carries the doc-style guidance comment. The mechanical reference sections (module-map / file-layout, dependency table) move inside generated-section fences and are derived from source, not hand-maintained.
  3. **Verify nothing was lost (the safety net, already wired).** The **D.13 pre-ship merge gate** catches a missed graduation while a dossier is still on disk; the **C.9 auditor git-aware graduation check** is the after-the-fact backstop that reads git history + the standing docs and emits **blocking** if a merged feature's durable bit never reached its home. The migration relies on these existing gates — it adds no new one.
  4. Tell the PM in plain language: "Your project docs are now lean and current — each one says what is true today, not the history of how it got there (that history is in git). Every important decision, contract, deferred item, and stack rule is still here, just kept in one clear place instead of scattered across per-feature files. Nothing was lost."
