...
- **Missing `.ai-pm/review-config.md` (cross-model review not configured):** an existing downstream project with **no** `.ai-pm/review-config.md` file. Per the `absent file OR unrecognized ⇒ session` default in `### Cross-model review` in `workflow/review-typology.md`, this falls safely to the session model — **review and audit run exactly as before, nothing changes by default** (non-disruptive). This is **not blocking and not a stale-artifact rewrite** — it is an **offer**, never forced: `/pm-audit` or the next `/pm-plan` *offers* to set up the config (the same bootstrap cross-model question), but an absent file is a fully valid, supported state. The migration is purely additive and back-compat-safe — no existing review behavior changes unless the PM opts in. Detect by the **absence** of `.ai-pm/review-config.md`; a project that already has the file is **not** flagged. The remediation is the **cross-model review setup offer** below.
...
- **Cross-model review setup offer — absent `.ai-pm/review-config.md` → offer to configure (never forced).** A downstream project with no `.ai-pm/review-config.md` is **offered** the cross-model review config; it is never auto-created and never blocking. Detection: see `### Pending-migration detection` above (absent `.ai-pm/review-config.md`). When it applies:

  **This is an additive opt-in offer, not a stale-artifact rewrite.** By the `absent file OR unrecognized ⇒ session` default in `### Cross-model review` in `workflow/review-typology.md`, the absent file already means "review and audit on the session model" — today's behavior. The offer only lets the PM opt into cross-model; declining (or never being asked) leaves the project fully functional.

  1. **Offer in plain language** (from `/pm-audit` or the next `/pm-plan`): "Right now review and audit run on your session model — same as always. You can optionally run them on a *different* model for independent blind spots (a different model catches what the maker-model misses). Want to set that up? You can keep the session model (default) or pick `auto` / `opus` / `sonnet`." PM decides.
  2. **On yes**, ask the cross-model question (the same one as `/pm-bootstrap` Q9) and write `.ai-pm/review-config.md` (the three model settings + `review-scope: auto`) per `### Cross-model review`. On no, write nothing — the absent-file default stands; do not re-nag every run.
  3. **Idempotent.** A project that already has `.ai-pm/review-config.md` is not detected, so the offer never fires twice. Running detection again on an opted-out project re-offers only on an explicit PM request, not proactively on every audit.
