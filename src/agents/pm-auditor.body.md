...
- **Readability-grade** (the carried plain-language smell, Slice A `### Plain language / human-readable`) — a **measurable readability-grade** signal on the standing docs (a Flesch-Kincaid-style grade level, target ≤ ~9–10). This is a **metric over sentence length and word length**, frame it that way and report it as such; it is **NOT** prose-policing — it never judges whether the prose is good, clear, or correct, only that the sentence/word-length metric sits above the target → **note**. Reference `### Plain language / human-readable` in `workflow/doc-style.md` by name; the legibility *act* (read-before-ship) stays the orchestrator's, not the auditor's.
