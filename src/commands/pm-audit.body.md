...
Present **one** structured-question-tool menu before running anything — the protocol's computed judgement (step 1) is the **recommended / pre-selected** option:
...
When the PM picks **Full**, also resolve the review-sweep **depth** in the same flow — fold it into this menu (a second question: light `low`/`medium` vs deep `high`/`max`) **or** defer to the `## Technical quality` depth offer, whichever keeps it to a single pass. Do **not** double-ask depth.
...
3. **Tell PM the headline:**
   ...
   If scope was `diff` — after the headline, ask PM via the structured-question tool:
...
6. **Run the chosen remediations** in priority order. One at a time:
   ...
   - Over-scale standing doc (size tripped the scale-guard) — the auditor's frugality-dimension note (a standing doc grown past its soft size target by a wide margin — `docs/architecture.md`, `docs/user-journeys.md`, or another standing doc no longer fits one readable pass) → offer to **shard** it now, in plain language. The fix splits the bloated doc into a thin core that points at on-demand sub-files — the same thin-core + on-demand structure `WORKFLOW.md` uses — and is a **compaction**, not new content:
     > "One of your reference docs has grown too big to read in one pass. I can split it into a short core that links out to focused sub-files — same structure as the main workflow doc, nothing removed, just reorganized so each piece is holdable. Run it now?"

     PM decides. On yes, spawn `pm-architect` (`subagent_type: "pm-architect"`, the doc-owner) to perform the **shard as a compaction remediation** — split the standing doc into a thin core + on-demand sub-files, move-not-delete (every section preserved). The auditor only flagged it; the orchestrator routes it to `pm-architect`.
   - Never-graduated merged feature (the git-aware graduation check, **blocking**) — the auditor's frugality-dimension blocking finding (a merged feature whose transient dossier evaporated **without** its durable knowledge graduating into one of the four homes — a decision missing from `docs/architecture.md`, a deferred finding never backlogged, a stack rule never recorded). This is silent knowledge loss; the durable bit's only copy is in git and nothing re-reads it as canon → spawn `pm-architect` (`subagent_type: "pm-architect"`) to **graduate the durable bit** into its home (decision record / `.ai-pm/contracts/` / `.ai-pm/backlog.md` / `docs/stack-notes.md`), recovering it from git history. (`accept-with-context` remains the conscious-defer escape hatch if the PM judges the bit not worth recovering.) The check tolerates a mixed old/new project — a bit present in a surviving old per-feature file already satisfies graduation, so this fires only on a genuine miss.
...
After the protocol findings are walked through, if scope was `full`, run the **smell / hygiene sweep** — the first concrete review **type** of `### Review typology` in `workflow/review-typology.md`. **Read `workflow/review-typology.md` before running the sweep** (the registry; do not re-encode the type list, cadence, or engine-selection rule here — reference it by name). Before spawning the sweep subagent, apply **deployment-context enrichment** per `### Deployment-context triage` in `workflow/review-typology.md` — read `docs/architecture.md` `## Operational limits & budgets` + `## Architectural constraints` (and `docs/threat-model.md` when present) and include them in the subagent prompt so the reviewer calibrates severity for the actual deployment target. It is a **whole-codebase** review type, so its engine is **selected per the engine-selection rule in `### Review typology`**: prefer `wb-development:code-review-orchestrator` when it is available and `WB_REVIEW_ORCHESTRATOR` is not `off`, otherwise fall back to the per-diff review engine (the compact reviewer / built-in `code-review`) run over the whole tree at the selected depth (below). The chosen engine runs over the **proportional scope** to surface functionality-preserving hygiene issues (dead code, duplication, high cognitive complexity, over-complexity, simplification) — distinct from a per-diff bug.

**Which *model* the sweep runs on — `review-full-model` (`### Cross-model review` in `workflow/review-typology.md`).** Orthogonal to engine selection above (engine = which reviewer; model = which brain). Resolve `review-full-model` per `### Cross-model review` (read `.ai-pm/review-config.md` fresh — absent/unrecognized ⇒ the rule's default). **`review-scope` does NOT apply here** — it gates the per-diff path (`review-diff-model`) only; this whole-codebase sweep uses `review-full-model` regardless of `review-scope`. Apply the resolved model over the existing engine chain, **model-pinned where the engine is ours**:

- **`code-review-orchestrator` available** → run it **inside a subagent pinned to the resolved model** and announce it. **Verify-before-rely caveat** (per `### Cross-model review`): the orchestrator skill's **fan-out** subagents inheriting the pinned model is a **to-verify, not an asserted fact** — so the announce must reflect **only what is actually pinned** (the top-level subagent), never a blanket "all on Sonnet" while the fan-out may still run on the session.
- **No orchestrator** → the **per-diff review engine run over the whole tree** (the compact reviewer where the harness has no built-in, the built-in `code-review` engine where it ships one) — **pinnable** to `review-full-model`, so cross-model works here without the orchestrator. This is a single path (no breadth-vs-pinned fork): the whole-tree fallback is always pinnable.

When the resolved model is `session` / equals the session / unavailable, run on the session and announce the fallback. The full sweep is **always pinnable** to `review-full-model` now (no non-pinnable path). The engine-selection + depth behavior below is **unchanged** — this adds only the model. Read the rule by name; do not re-encode the enum/default/Haiku-blacklist/fallback/fan-out-caveat. Its deterministic **detection** half (an enumerable smell catalog a hook/linter should own) is a **named downstream/future path, not run here** (this protocol repo has no linter to host it — see `### Review typology`); the AI **prioritization / root-cause** half is this sweep.
...
**3. Selectable depth (never silently the costliest).** Run the selected engine (step intro — orchestrator preferred when available, else the per-diff review engine over the whole tree) at a **chosen** depth — never silently the costliest. Depth applies to the fallback engine's level (low / medium / high / max); the orchestrator runs its own multi-aspect pass:

- **Interactive:** offer the cost/depth trade-off and let the PM pick:
  > "Protocol check done. Want a smell/hygiene sweep too? It reviews the code changed since the last sweep (or the full tree on a first / periodic re-sweep) for dead code, duplication, and over-complexity. Depth is selectable — light (`low`/`medium`, a few minutes) for a small routine sweep, deep (`high`/`max`, 10–15 min) for a first/legacy/comprehensive sweep. Run it — and at what depth?"
