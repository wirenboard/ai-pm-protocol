...
Install glue is in `INSTALL.md` (where each file lands, the Claude hook fragment, the OpenCode plugin entry). The OpenCode entry defines its hook functions inline and imports only the rule logic — a hook imported and re-exported is not registered by the loader. Both shims are documented in `INSTALL.md`; the per-class support is in `tool-map.json`.
