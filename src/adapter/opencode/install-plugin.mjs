...
//   • downstream: <root>/.ai-pm/tooling/src/adapter  (the source's own path → no rewrite)
//   • this repo / dev: <root>/src/adapter             (rewrite .ai-pm/tooling/src/adapter → src/adapter)
...
const SOURCE_IMPORT_DIR = "../../../.ai-pm/tooling/src/adapter"; // the two import specifiers' prefix
const SOURCE_RESOLVE_SEGS = '"..", "..", "..", ".ai-pm", "tooling", "src", "adapter"'; // the path.resolve segments
...
function resolveRewrite(root, layout) {
  const resolved =
    layout ||
    (fs.existsSync(path.join(root, ".ai-pm", "tooling", "src", "adapter"))
      ? "downstream"
  ...
  throw new Error(
    `install-plugin: no adapter dir found under ${root} (looked for .ai-pm/tooling/src/adapter and src/adapter)`,
  );
}
